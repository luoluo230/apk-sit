...
def _sanitize_settings_file(path: Path, mode: str, default_port: int):
    ...
    if isinstance(jenkins_cfg, dict):
        ...
        jenkins_cfg.setdefault("image", "jenkins/jenkins:lts-jdk17")
        jenkins_cfg.setdefault("container_name", "apk-site-jenkins")
...
def _rewrite_env_example(path: Path, mode: str, default_port: int):
    ...
    replacements = {
        ...
        "JENKINS_CONTAINER_NAME": "apk-site-jenkins",
        "PLAYER_PUBLIC_URL": "",
        "FORUM_PUBLIC_URL": "",
        "ADMIN_PUBLIC_URL": "",
        "PLAYER_DOMAIN": "",
...
def _runtime_readme(mode: str, default_port: int, script_name: str, include_jenkins: bool):
    sections = [
        f"# {mode} bundle",
        "",
        ...
        "```bat",
        f".\\{script_name}.bat -CheckOnly",
        "```",
        "",
        "## Custom port",
        "```bat",
        f".\\{script_name}.bat -Port {default_port}",
        "```",
        "",
        "## Disable auto Python install",
        "```bat",
        f".\\{script_name}.bat -InstallPythonIfMissing:$false",
        "```",
    ]
    ...
    if include_jenkins:
        sections.extend(
            [
                "",
                "## Jenkins build runtime bootstrap (admin backend only)",
                "```bat",
...
def _build_runtime_bundle(target_dir: Path, mode: str, default_port: int):
    ...
    _write_text(
        ...
        _runtime_readme(mode=mode, default_port=default_port, script_name=script_name, include_jenkins=include_jenkins),
    )
