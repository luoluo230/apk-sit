...
from flask import Blueprint, request, jsonify, render_template, render_template_string, session, send_from_directory
from services.authz import login_required, admin_required, get_visible_modules, ADMIN_MODULES, ALL_MODULES_EXCEPT_USER_MANAGEMENT, is_super_admin_or_admin, has_scope
...
MODULE_LINKS = {
    ...
    'dashboard': ('/dashboard', 'fa-chart-pie', 'text-purple-500'),
...
@bp.route('/uploaded-media/<path:relative_path>')
def uploaded_media_file(relative_path):
    base = media_root()
    clean_relative = os.path.normpath(str(relative_path or '').replace('\\', '/')).replace('\\', '/')
    if clean_relative.startswith('..'):
        return '', 404
    return send_from_directory(base, clean_relative, as_attachment=False)


@bp.route('/admin/media/upload', methods=['POST'])
@login_required
def admin_media_upload():
    scope = (request.form.get('scope') or 'shared').strip() or 'shared'
    bucket = (request.form.get('project_id') or request.form.get('bucket') or 'shared').strip() or 'shared'
    files = request.files.getlist('files')
    if not files:
        single = request.files.get('file')
        if single:
            files = [single]
    uploaded = save_uploaded_files(files, scope=scope, bucket=bucket)
    if not uploaded:
        return jsonify({'error': '请选择本地图片或视频文件'}), 400
    return jsonify({'ok': True, 'files': uploaded, 'file': uploaded[0]})
        ...
        <div class="rounded-[28px] border border-slate-200/80 bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-900 p-6 md:p-7 shadow-xl shadow-slate-900/10"> 
            <div class="grid gap-6 xl:grid-cols-[1.45fr_0.95fr] xl:items-start"> 
                <div>
                    ...
                    <div class="mt-5 flex flex-wrap gap-2">''' + (''.join(quick_actions) if quick_actions else '<span class="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-2 text-sm text-slate-300">当前没有快捷操作</span>') + '''</div>
                </div>
                <div class="grid gap-3 sm:grid-cols-2">''' + ''.join(
                    f'<div class="rounded-2xl border border-white/10 bg-white/5 p-4 backdrop-blur"><div class="flex items-center justify-between gap-3"><div><p class="text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-300">{label}</p><p class="mt-2 text-3xl font-semibold text-white">{value}</p></div><div class="flex h-11 w-11 items-center justify-center rounded-2xl bg-white/10 text-white"><i class="fas {icon} text-lg"></i></div></div></div>'
        ...
        <div class="grid gap-5 xl:grid-cols-[1.55fr_0.95fr]"> 
            <div class="space-y-5"> 
                ...
                <div class="rounded-2xl border border-slate-200/80 bg-white/95 p-5 shadow-sm"> 
                    <div class="mb-4 flex flex-wrap items-center justify-between gap-3"> 
                        <div>
                            <h3 class="text-base font-semibold text-slate-900">角色化模块入口</h3>
...
def admin_search():
    ...
    proj_rows = ''.join('<tr><td class="px-4 py-2"><a href="%s" class="text-indigo-600 hover:underline">%s</a></td><td class="px-4 py-2 text-sm text-gray-500">%s</td></tr>' % (html.escape(p['link']), html.escape(p['name']), html.escape(p['id'])) for p in results['projects'][:20])
    task_rows = ''.join('<tr><td class="px-4 py-2"><a href="%s" class="text-indigo-600 hover:underline">%s</a></td><td class="px-4 py-2 text-sm">%s</td></tr>' % (html.escape(t['link']), html.escape(t['title']), html.escape(t['project_id'])) for t in results['tasks'][:20])
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <form method="get" action="/admin/search" class="p-4 border-b flex gap-2">
            <input type="text" name="q" value="''' + html.escape(q) + '''" placeholder="搜索项目、任务、用户…" class="flex-1 px-4 py-2 border border-gray-200 rounded-lg text-sm">
            <button type="submit" class="px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium">搜索</button>
        </form>
...
def _admin_panel_dashboard(visible, desc):
    ...
    has_projects = any(m[0] == 'projects' for m in visible_sorted)
    grouped_cards = {
        '运维中心': [],
        '运营中心': [],
        '开发中心': [],
    ...
    module_groups = {
        ...
        'projects': '开发中心',
        'docs': '开发中心',
        'user_management': '配置中心',
        'system_settings': '配置中心',
        'audit_log': '配置中心',
    }
    for mid, name, _ in visible_sorted:
        link, icon, color = MODULE_LINKS.get(mid, ('#', 'fa-circle', 'text-gray-500'))
        group = module_groups.get(mid, '开发中心')
        grouped_cards.setdefault(group, []).append(
            _render_module_card(link, icon, color, name, desc.get(mid, ''))
        )
    if has_projects:
        grouped_cards['开发中心'].append(
            _render_module_card('/admin/my-tasks', 'fa-tasks', 'text-indigo-500', '我的任务', '查看并处理分配给自己的项目任务')
        )
    grouped_cards['开发中心'].append(
        _render_module_card('/admin/products', 'fa-gamepad', 'text-emerald-500', '产品管理', '统一维护游戏官网展示、媒体素材、项目归属与对外入口')
    )
    grouped_cards['配置中心'].append(
        _render_module_card('/workspace', 'fa-briefcase', 'text-amber-500', '个人工作区', '处理个人文件、截图、书签和协作资料')
    )
    grouped_cards['配置中心'].append(
        _render_module_card('/admin/site-config', 'fa-swatchbook', 'text-fuchsia-500', '官网与外部模块配置', '统一维护公司简介、玩家官网、开发者官网与外部模块入口')
    )
    cards = _render_grouped_admin_sections([
        ('运维中心', '构建、分发、监控与报表能力集中在这里。', grouped_cards.get('运维中心')),
        ('运营中心', '新闻、福利、论坛官方内容、审批与通知在同一操作区。', grouped_cards.get('运营中心')),
        ('开发中心', '项目、版本、产品与个人任务围绕研发交付组织。', grouped_cards.get('开发中心')),
        ('配置中心', '官网配置、用户权限、系统设置与审计入口统一收口。', grouped_cards.get('配置中心')),
    ])
    ...
    if pending_approvals:
        quick_actions.append(f'<a href="/admin/approval" class="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-amber-50 text-amber-800 border border-amber-100 text-sm font-medium hover:bg-amber-100">待审批 {len(pending_approvals)} 项</a>')
    if unread_notifications:
        quick_actions.append(f'<a href="/admin/notifications" class="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-sky-50 text-sky-800 border border-sky-100 text-sm font-medium hover:bg-sky-100">未读通知 {len(unread_notifications)} 条</a>')
    if overdue_tasks:
        quick_actions.append(f'<a href="/admin/my-tasks?quick=overdue" class="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-rose-50 text-rose-800 border border-rose-100 text-sm font-medium hover:bg-rose-100">超时任务 {len(overdue_tasks)} 条</a>')
    if missing_package_versions:
        quick_actions.append(f'<a href="/admin/projects" class="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-violet-50 text-violet-800 border border-violet-100 text-sm font-medium hover:bg-violet-100">缺安装包版本 {len(missing_package_versions)} 个</a>')
    ...
    todo_html = ''.join(
        f'<li class="flex items-start justify-between gap-3"><span class="text-sm text-slate-700">审批：{html_module.escape(item.get("type",""))} / {html_module.escape(item.get("target_id",""))}</span><a href="/admin/approval" class="text-xs text-indigo-600 hover:underline">去处理</a></li>'
        for item in pending_approvals[:3]
    )
    todo_html += ''.join(
        f'<li class="flex items-start justify-between gap-3"><span class="text-sm text-slate-700">超时任务：{html_module.escape(_clean_display_text(item.get("title"), "未命名任务"))}</span><a href="/admin/my-tasks?quick=overdue" class="text-xs text-indigo-600 hover:underline">去处理</a></li>'
        for item in overdue_tasks[:3]
    )
    todo_html += ''.join(
        f'<li class="flex items-start justify-between gap-3"><span class="text-sm text-slate-700">通知：{html_module.escape(_clean_display_text(item.get("title"), "未命名通知"))}</span><a href="/admin/notifications" class="text-xs text-indigo-600 hover:underline">查看</a></li>'
        for item in unread_notifications[:2]
    )
    if not todo_html:
        todo_html = '<li class="text-sm text-slate-500">当前没有待处理事项</li>'
    risk_html = ''.join(
        f'<li class="flex items-start justify-between gap-3"><span class="text-sm text-slate-700">{html_module.escape(item["project_id"])} / {html_module.escape(item["version_name"])}</span><span class="text-xs px-2 py-0.5 rounded bg-slate-100 text-slate-600">{html_module.escape(item["platform_label"])}</span></li>'
        for item in missing_package_versions[:4]
    )
    risk_html += ''.join(
        f'<li class="flex items-start justify-between gap-3"><span class="text-sm text-slate-700">{html_module.escape(project_id)} 缺少推荐版本</span><span class="text-xs px-2 py-0.5 rounded bg-amber-100 text-amber-700">建议处理</span></li>'
        for project_id in recommended_gaps[:4]
    )
    if not risk_html:
        risk_html = '<li class="text-sm text-slate-500">当前没有明显版本风险</li>'
    audit_html = ''.join(
        f'<li class="flex items-start justify-between gap-3"><div><p class="text-sm font-medium text-slate-800">{html_module.escape(_clean_display_text(entry.get("action"), "系统操作"))}</p><p class="text-xs text-slate-500">{html_module.escape(_clean_display_text(entry.get("user"), "-"))} · {html_module.escape((entry.get("at") or entry.get("timestamp") or "")[:16])}</p></div><span class="text-xs text-slate-400 max-w-[200px] truncate">{html_module.escape(_clean_display_text(entry.get("details"), "无附加说明")[:40])}</span></li>'
...
def admin_site_config_page():
                ...
                <div class="rounded-2xl border border-slate-200 bg-white p-4 space-y-3 cursor-move" data-module-row draggable="true">
                    <div class="grid gap-3 md:grid-cols-2">
                        <div>
                            ...
                            <select class="module-type mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm">{_module_type_options(mtype)}</select>
                        </div>
                        <div>
                            <label class="text-xs font-semibold text-slate-500">模块标题</label>
                            <input type="text" value="{title}" class="module-title mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" placeholder="展示标题">
                        </div>
                    </div>
                    <div class="grid gap-3 md:grid-cols-3">
                        ...
                        <div>
                            ...
                            <input type="number" value="{order}" min="1" class="module-order mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm">
                        </div>
                        <div>
                            <label class="text-xs font-semibold text-slate-500">数量限制</label>
                            <input type="number" value="{limit}" min="1" class="module-limit mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" placeholder="可选">
                        </div>
                    </div>
                    <div>
                        <label class="text-xs font-semibold text-slate-500">模块描述</label>
                        <textarea class="module-description mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" rows="2" placeholder="模块说明">{description}</textarea>
                    </div>
                    <div class="grid gap-3 md:grid-cols-2">
                        <div>
                            <label class="text-xs font-semibold text-slate-500">样式</label>
                            <select class="module-size mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm">{_size_options(size)}</select>
                        </div>
                        <div>
                            <label class="text-xs font-semibold text-slate-500">数据来源</label>
                            <input type="text" value="{source}" class="module-source mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" placeholder="如 latest / featured">
                        </div>
                    </div>
                    <div class="grid gap-3 md:grid-cols-2">
                        <div>
                            <label class="text-xs font-semibold text-slate-500">本地图片</label>
                            <input type="text" value="{image_url}" class="module-image-url mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" placeholder="上传后自动填充本地路径" readonly>
                        </div>
                        <div>
                            <label class="text-xs font-semibold text-slate-500">本地视频</label>
                            <input type="text" value="{video_url}" class="module-video-url mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" placeholder="上传后自动填充本地路径" readonly>
                        </div>
                    </div>
                    <div>
                        <label class="text-xs font-semibold text-slate-500">本地图集</label>
                        <textarea class="module-media-urls mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" rows="2" placeholder="上传后自动填充本地路径，支持多张" readonly>{media_urls}</textarea>
                    </div>
                    <div class="grid gap-3 md:grid-cols-2">
                        <div>
                            <label class="text-xs font-semibold text-slate-500">CTA 文案</label>
                            <input type="text" value="{cta_text}" class="module-cta-text mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" placeholder="按钮文案">
                        </div>
                        <div>
                            <label class="text-xs font-semibold text-slate-500">CTA 链接</label>
                            <input type="text" value="{cta_link}" class="module-cta-link mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" placeholder="按钮链接">
                        </div>
                    </div>
    ...
    <section class="space-y-6">
        <div class="flex items-end justify-between gap-4">
            ...
            <div class="flex items-center gap-3">
                <a href="/admin/site-config/editor/player" class="rounded-xl border border-violet-200 bg-violet-50 px-4 py-2 text-sm font-semibold text-violet-700">\u73a9\u5bb6\u5b98\u7f51\u53ef\u89c6\u5316\u7f16\u8f91\u5668</a>
                <a href="/admin/site-config/editor/dev" class="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-2 text-sm font-semibold text-emerald-700">\u5f00\u53d1\u8005\u5b98\u7f51\u53ef\u89c6\u5316\u7f16\u8f91\u5668</a>
                <a href="/admin" class="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700">\u8fd4\u56de\u7ba1\u7406\u4e2d\u5fc3</a>
            </div>
        </div>
                ...
                <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                    ...
                    <div class="mt-3 space-y-3">
                        <div>
                            ...
                            <select name="visible_product_ids" multiple class="mt-2 h-28 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm">{player_options}</select>
                            <p class="mt-2 text-xs text-slate-500">\u672a\u9009\u62e9\u5219\u5c55\u793a\u5168\u90e8\u9879\u76ee</p>
                        </div>
                        <div>
                            <label class="text-xs font-semibold text-slate-500">\u4e3b\u63a8\u4ea7\u54c1\uff08\u591a\u9009\uff09</label>
                            <select name="featured_product_ids" multiple class="mt-2 h-24 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm">{player_featured_options}</select>
            ...
            <form id="devPortalConfigForm" class="space-y-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                    ...
                    <div class="mt-3 space-y-3">
                        <div>
                            <label class="text-xs font-semibold text-slate-500">\u5c55\u793a\u4ea7\u54c1\uff08\u591a\u9009\uff09</label>
                            <select name="visible_product_ids" multiple class="mt-2 h-28 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm">{dev_options}</select>
                            <p class="mt-2 text-xs text-slate-500">\u672a\u9009\u62e9\u5219\u5c55\u793a\u5168\u90e8\u9879\u76ee</p>
                        </div>
                        <div>
                            <label class="text-xs font-semibold text-slate-500">\u4e3b\u63a8\u4ea7\u54c1\uff08\u591a\u9009\uff09</label>
                            <select name="featured_product_ids" multiple class="mt-2 h-24 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm">{dev_featured_options}</select>
                            <p class="mt-2 text-xs text-slate-500">\u9996\u4e2a\u4e3b\u63a8\u4ea7\u54c1\u5c06\u4f5c\u4e3a\u9996\u9875\u4e3b\u89c6\u89c9</p>
                        </div>
                    </div>
                </div>
...
@bp.route('/admin/site-config/company', methods=['POST'])
@admin_required()
def admin_site_config_save_company():
    save_company_profile(request.get_json(silent=True) or {})
    return jsonify({'ok': True})


@bp.route('/admin/site-config/portal/player', methods=['POST'])
@admin_required()
def admin_site_config_save_player_portal():
    save_player_portal_content(request.get_json(silent=True) or {})
    return jsonify({'ok': True})


@bp.route('/admin/site-config/portal/dev', methods=['POST'])
@admin_required()
def admin_site_config_save_dev_portal():
    save_dev_portal_content(request.get_json(silent=True) or {})
    return jsonify({'ok': True})
...
@bp.route('/admin/site-config/editor/<portal_kind>/save', methods=['POST'])
@admin_required()
def admin_site_config_visual_editor_save(portal_kind):
    if portal_kind not in ('player', 'dev'):
        return jsonify({'error': '未知官网类型'}), 400

    payload = request.get_json(silent=True) or {}
    modules = _visual_editor_normalize_modules(payload.get('home_modules'), portal_kind)
    ...
    if portal_kind == 'player':
        save_player_portal_content(cleaned_payload)
    else:
        save_dev_portal_content(cleaned_payload)
    return jsonify({'ok': True, 'message': '可视化布局已保存'})
    ...
    if has_projects:
        cards.append(
            ...
            '<div class="flex items-start gap-3"><div class="w-10 h-10 rounded-xl bg-indigo-50 flex items-center justify-center flex-shrink-0">'
    ...
    cards.append(
        ...
        '<div class="flex items-start gap-3"><div class="w-10 h-10 rounded-xl bg-amber-50 flex items-center justify-center flex-shrink-0">'
    ...
    if is_super_admin_or_admin():
        cards.append(
            ...
            '<div class="flex items-start gap-3"><div class="w-10 h-10 rounded-xl bg-emerald-50 flex items-center justify-center flex-shrink-0">'
    ...
    <div class="px-6 py-4 border-b border-gray-100 flex flex-wrap items-center justify-between gap-3 bg-gray-50/50">
        ...
        <div class="flex flex-wrap items-center gap-2">
            <div class="relative">
                ...
                <input type="text" id="userSearch" placeholder="搜索用户名、角色…" class="pl-8 pr-3 py-2 border border-gray-200 rounded-lg text-sm w-48 focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
            </div>
            ...
            <div class="grid gap-6 lg:grid-cols-3 items-start">
                <div class="lg:col-span-1 space-y-4">
                    <div>
                        ...
                        <input type="text" id="newUsername" placeholder="用户名" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-500 mb-1">密码（至少 {{PASSWORD_MIN}} 位）</label>
                        <input type="password" id="newPassword" placeholder="密码" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500" required minlength="{{PASSWORD_MIN}}">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-500 mb-1">角色</label>
                        <select id="newRole" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500">
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden" onclick="event.stopPropagation()">
        ...
        <div class="px-6 py-5 space-y-4">
            <div>
                ...
                <input type="text" id="resetPwdNew" placeholder="输入新密码" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500">
...
def admin_users_list():
    ...
    for k, v in users_db.items():
        users.append({
            ...
            'allowed_modules': v.get('allowed_modules') or [],
            'disabled': v.get('disabled', False),
        })
    return jsonify({'users': users})


@bp.route('/admin/users/get/<username>')
@admin_required('user_management')
def admin_users_get(username):
    ...
    return jsonify({
        'user': {
            ...
            'role': v.get('role', 'user'),
            'allowed_modules': v.get('allowed_modules') or [],
            'allowed_scopes': v.get('allowed_scopes') or [],
            'disabled': v.get('disabled', False),
        }
    })
...
def admin_users_create():
    ...
    if len(password) < min_len:
        return jsonify({'error': '密码至少 %d 位' % min_len})
    if username in users_db:
        return jsonify({'error': '用户已存在'})
    role = role if role in ('admin', 'super_admin', 'user') else 'user'
    rec = {
    ...
    if role == 'user':
        ...
        if 'allowed_scopes' in data:
            ...
            rec['allowed_scopes'] = scopes if isinstance(scopes, list) else [s.strip() for s in str(scopes).split(',') if s.strip()]
...
def admin_users_update():
    ...
    if not username or username not in users_db:
        return jsonify({'error': '用户不存在'})
    if username == 'admin':
        return jsonify({'error': '不能修改 admin'})
    u = users_db[username]
    if u.get('role') == 'super_admin':
        return jsonify({'error': '不能修改超级管理员'})
    if 'allowed_modules' in data:
        u['allowed_modules'] = data['allowed_modules'] if isinstance(data['allowed_modules'], list) else []
    if 'allowed_scopes' in data:
        scopes = data['allowed_scopes']
        if isinstance(scopes, list):
            u['allowed_scopes'] = scopes
        else:
            try:
                u['allowed_scopes'] = [s.strip() for s in str(scopes).split(',') if s.strip()]
            except Exception:
                u['allowed_scopes'] = []
    if 'disabled' in data:
        u['disabled'] = bool(data['disabled'])
    save_users()
    log_audit('update_user', username)
    return jsonify({'success': True})
...
def admin_users_reset_password():
    ...
    if not username or username not in users_db:
        return jsonify({'error': '用户不存在'})
    if username == 'admin':
        return jsonify({'error': '不能修改 admin 密码'})
    if users_db[username].get('role') == 'super_admin':
        return jsonify({'error': '不能修改超级管理员密码'})
    if not new_password:
        return jsonify({'error': '新密码不能为空'})
    if len(new_password) < _password_min_length():
        return jsonify({'error': '密码至少 %d 位' % _password_min_length()})
    users_db[username]['password'] = hashlib.sha256(new_password.encode()).hexdigest()
    save_users()
    log_audit('reset_password', username)
    return jsonify({'success': True})


@bp.route('/admin/users/delete/<username>', methods=['DELETE'])
@admin_required('user_management')
def admin_users_delete(username):
    if username == 'admin':
        return jsonify({'error': '不能删除 admin'})
    if username not in users_db:
        return jsonify({'error': '用户不存在'})
    if users_db[username].get('role') == 'super_admin':
    ...
    save_users()
    log_audit('delete_user', username)
    return jsonify({'success': True})
...
def admin_users_export():
    ...
    w.writerow(['用户名', '角色', '禁用', '模块权限', '操作权限', '创建时间', '最后登录'])
    for uname, u in sorted(users_db.items()):
        mods = u.get('allowed_modules') or []
        mod_str = ','.join(mods) if isinstance(mods, list) else str(mods)
        scopes = u.get('allowed_scopes') or []
        scopes_str = ','.join(scopes) if isinstance(scopes, list) else str(scopes)
        w.writerow([
            uname,
            u.get('role', 'user'),
            '是' if u.get('disabled') else '否',
            mod_str,
            scopes_str,
    ...
    fn = 'users_export_%s.csv' % datetime.now().strftime('%Y%m%d_%H%M')
    return send_file(io.BytesIO(buf.getvalue().encode('utf-8-sig')), as_attachment=True, download_name=fn, mimetype='text/csv')
...
def admin_users_import():
    ...
    try:
        ...
        try:
            ...
            return jsonify({'error': '文件编码不支持，请使用 UTF-8 或 GBK'})
    reader = csv_module.reader(io.StringIO(content))
    rows = list(reader)
    if not rows:
        return jsonify({'error': 'CSV 为空'})
    ...
    for row in rows[1:]:
        if len(row) <= max(username_idx, password_idx):
            continue
        uname = (row[username_idx] or '').strip()
        pwd = (row[password_idx] or '').strip()
        role = (row[role_idx] or 'user').strip() if role_idx < len(row) else 'user'
        if not uname:
            continue
        if uname in ('admin',) or users_db.get(uname, {}).get('role') == 'super_admin':
            skipped += 1
            continue
        ...
        if role == 'user':
            users_db[uname]['allowed_modules'] = ['*']
        created += 1
    save_users()
    log_audit('users_import', 'created=%d skipped=%d' % (created, skipped))
...
<section class="space-y-5">
    <div class="flex items-end justify-between gap-3">
        ...
        <div class="flex items-center gap-2">
            <select id="projectStatusFilter" onchange="loadProjects()" class="px-3 py-2 border border-slate-200 rounded-xl text-sm bg-white shadow-sm">
                <option value="active">仅活跃</option>
                <option value="archived">仅归档</option>
                <option value="">全部</option>
            </select>
            <input type="text" id="projectSearch" placeholder="搜索项目ID、名称、英文名…" class="px-3 py-2 border border-slate-200 rounded-xl text-sm w-56 focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500 bg-white shadow-sm">
            <button type="button" onclick="openChannelManageModal()" class="inline-flex items-center px-3.5 py-2 rounded-xl border border-slate-200 bg-white text-sm text-slate-700 hover:bg-slate-50 shadow-sm">
    ...
    <div id="addProjectForm" class="px-6 py-5 border border-slate-200 rounded-2xl bg-white/95 shadow-sm hidden">
        <form onsubmit="return false;" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div><label class="block text-xs font-medium text-slate-500 mb-1">项目ID（英文）</label><input type="text" id="newProjectId" placeholder="如 MyGame" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500" required></div>
                <div><label class="block text-xs font-medium text-slate-500 mb-1">名称</label><input type="text" id="newProjectName" placeholder="项目名称" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500" required></div>
                <div><label class="block text-xs font-medium text-slate-500 mb-1">英文名</label><input type="text" id="newProjectNameEn" placeholder="English name" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500"></div>
                ...
                <div><label class="block text-xs font-medium text-slate-500 mb-1">项目阶段</label><select id="newProjectPhase" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500">''' + ''.join('<option value="%s">%s</option>' % (k, v) for k, v in PROJECT_PHASES) + '''</select></div>
            </div>
            <div><label class="block text-xs font-medium text-slate-500 mb-1">简介</label><input type="text" id="newProjectIntro" placeholder="简短介绍" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500"></div>
            <div><label class="block text-xs font-medium text-slate-500 mb-1">详情</label><textarea id="newProjectDetail" rows="2" placeholder="详细描述" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500"></textarea></div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><label class="block text-xs font-medium text-slate-500 mb-1">网络连接说明</label><input type="text" id="newProjectNetwork" placeholder="如：需内网/外网" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500"></div>
                <div><label class="block text-xs font-medium text-slate-500 mb-1">Git 地址</label><input type="text" id="newProjectGitUrl" placeholder="https://或 git@" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500"></div>
                <div class="md:col-span-2"><label class="block text-xs font-medium text-slate-500 mb-1">GitHub/SSH Key 路径</label><input type="text" id="newProjectGitSshKey" placeholder="如 /Users/xxx/.ssh/id_rsa" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500"></div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><label class="block text-xs font-medium text-slate-500 mb-1">可查看用户</label><input type="text" id="newProjectViewers" placeholder="多个用户名用逗号分隔" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500"></div>
                <div><label class="block text-xs font-medium text-slate-500 mb-1">项目参与人员（可编辑）</label><div class="flex gap-2"><input type="text" id="newParticipantUser" placeholder="输入用户名" class="flex-1 px-3 py-2 border border-slate-200 rounded-lg text-sm"><button type="button" onclick="addNewParticipant()" class="px-3 py-2 bg-indigo-50 text-indigo-700 rounded-lg text-sm">验证并添加</button></div><ul id="newParticipantsList" class="mt-2 space-y-1 text-sm"></ul></div>
            </div>
            ...
            <table class="min-w-full">
                <thead class="bg-slate-50/80 border-b border-slate-200/80">
                    <tr>
                        ...
                        <th class="px-6 py-3.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">操作</th>
                    </tr>
                </thead>
...
<div id="channelManageModal" class="hidden fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onclick="if(event.target===this) this.classList.add('hidden')">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-hidden flex flex-col" onclick="event.stopPropagation()">
            ...
            <div class="border border-dashed border-slate-200 rounded-xl p-3 bg-slate-50/60">
                <div class="grid grid-cols-[1.2fr,1.2fr,0.6fr] gap-3 items-end">
                    <div>
                        ...
                        <input type="text" id="channelFormId" placeholder="如 dev / test / production" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-slate-600 mb-1">渠道名称</label>
                        <input type="text" id="channelFormName" placeholder="如 开发版 / 测试版 / 线上版" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-slate-600 mb-1">排序</label>
                        <input type="number" id="channelFormOrder" value="0" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500">
                    </div>
                </div>
                <div class="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div><label class="block text-xs font-medium text-slate-600 mb-1">APK 子目录（可选）</label><input type="text" id="channelFormApkSubdir" placeholder="如 dev、test" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500"></div>
                    <div><label class="block text-xs font-medium text-slate-600 mb-1">构建参数（可选）</label><input type="text" id="channelFormBuildParam" placeholder="如 CHANNEL=dev" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500"></div>
                </div>
                <div class="mt-3">
                    <label class="block text-xs font-medium text-slate-600 mb-1">说明（可选）</label>
                    <textarea id="channelFormDesc" rows="2" placeholder="用于标记该渠道的用途，例如“内部联调测试”、“线上正式发布”等" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500"></textarea>
                </div>
        ...
        <div class="px-6 py-3 border-t border-slate-100 bg-slate-50/80 flex justify-end">
            <button type="button" onclick="closeChannelManageModal()" class="px-4 py-2 text-sm text-slate-600 bg-white border border-slate-200 rounded-lg hover:bg-slate-50">关闭</button>
        </div>
    </div>
</div>
<div id="participantRoleModal" class="hidden fixed inset-0 bg-black/60 flex items-center justify-center z-[60] p-4" onclick="if(event.target===this) this.classList.add('hidden')">
    <div class="bg-white rounded-xl shadow-xl w-80 p-5" onclick="event.stopPropagation()">
        ...
        <select id="roleModalRole" class="w-full px-3 py-2 border rounded-lg text-sm mb-4">''' + ''.join('<option value="%s">%s</option>' % (r, r) for r in PROJECT_ROLES) + '''</select>
        <div class="flex gap-2"><button type="button" onclick="confirmParticipantRole()" class="flex-1 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm">确认</button><button type="button" onclick="document.getElementById('participantRoleModal').classList.add('hidden')" class="px-4 py-2 border rounded-lg text-sm">取消</button></div>
    </div>
</div>
<div id="editProjectModal" class="hidden fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onclick="if(event.target===this) this.classList.add('hidden')">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col" onclick="event.stopPropagation()">
        ...
        <div class="px-6 py-5 overflow-y-auto flex-1 space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><label class="block text-xs font-medium text-gray-500 mb-1">名称</label><input type="text" id="editProjectName" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500"></div>
                <div><label class="block text-xs font-medium text-gray-500 mb-1">英文名</label><input type="text" id="editProjectNameEn" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500"></div>
            </div>
            <div><label class="block text-xs font-medium text-gray-500 mb-1">项目阶段</label><select id="editProjectPhase" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500">''' + ''.join('<option value="%s">%s</option>' % (k, v) for k, v in PROJECT_PHASES) + '''</select></div>
            ...
            <div><label class="block text-xs font-medium text-gray-500 mb-1">简介</label><input type="text" id="editProjectIntro" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500"></div>
            <div><label class="block text-xs font-medium text-gray-500 mb-1">详情</label><textarea id="editProjectDetail" rows="2" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500"></textarea></div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><label class="block text-xs font-medium text-gray-500 mb-1">网络连接说明</label><input type="text" id="editProjectNetwork" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500"></div>
                <div><label class="block text-xs font-medium text-gray-500 mb-1">Git 地址</label><input type="text" id="editProjectGitUrl" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500"></div>
                <div class="md:col-span-2"><label class="block text-xs font-medium text-gray-500 mb-1">GitHub/SSH Key 路径</label><input type="text" id="editProjectGitSshKey" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500"></div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><label class="block text-xs font-medium text-gray-500 mb-1">可查看用户</label><input type="text" id="editProjectViewers" placeholder="多个用户名用逗号分隔" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500"></div>
                <div><label class="block text-xs font-medium text-gray-500 mb-1">项目参与人员（可编辑）</label><div class="flex gap-2"><input type="text" id="editParticipantUser" placeholder="输入用户名" class="flex-1 px-3 py-2 border border-gray-200 rounded-lg text-sm"><button type="button" onclick="addEditParticipant()" class="px-3 py-2 bg-indigo-100 text-indigo-700 rounded-lg text-sm">验证并添加</button></div><ul id="editParticipantsList" class="mt-2 space-y-1 text-sm"></ul></div>
            </div>
...
function renderProjectsTable(projects){
    ...
    t.innerHTML = list.map(function(p, i){
        ...
        var archiveBtn = canEdit && p.status!=='archived'
            ? '<button onclick="archiveProject(\\''+p.id+'\\', true)" class="px-2.5 py-1 text-xs text-slate-600 bg-slate-50 hover:bg-slate-100 rounded-lg">归档</button>'
            : (canEdit && p.status==='archived'
                ? '<button onclick="archiveProject(\\''+p.id+'\\', false)" class="px-2.5 py-1 text-xs text-emerald-700 bg-emerald-50 hover:bg-emerald-100 rounded-lg">取消归档</button>'
                : '');
        var actions = canEdit
            ? '<a href="/admin/projects/'+p.id+'" class="px-2.5 py-1 text-xs font-medium text-amber-700 bg-amber-50 hover:bg-amber-100 rounded-lg">项目中心</a>'
              + '<a href="/admin/projects/'+p.id+'/tasks" class="px-2.5 py-1 text-xs text-indigo-700 bg-indigo-50 hover:bg-indigo-100 rounded-lg">任务</a>'
              + '<a href="/admin/projects/'+p.id+'#tab=channels" class="px-2.5 py-1 text-xs text-emerald-700 bg-emerald-50 hover:bg-emerald-100 rounded-lg">版本</a>'
              + '<a href="/admin/projects/'+p.id+'/build" class="px-2.5 py-1 text-xs text-orange-700 bg-orange-50 hover:bg-orange-100 rounded-lg">构建</a>'
              + '<a href="/download-center?project='+encodeURIComponent(p.id)+'" class="px-2.5 py-1 text-xs text-teal-700 bg-teal-50 hover:bg-teal-100 rounded-lg">下载</a>'
              + '<button onclick="editProject(\\''+p.id+'\\')" class="px-2.5 py-1 text-xs text-blue-700 bg-blue-50 hover:bg-blue-100 rounded-lg">编辑</button>'
              + archiveBtn
              + '<button onclick="deleteProject(\\''+p.id+'\\')" class="px-2.5 py-1 text-xs text-red-700 bg-red-50 hover:bg-red-100 rounded-lg">删除</button>'
            : '<a href="/admin/projects/'+p.id+'" class="px-2.5 py-1 text-xs font-medium text-amber-700 bg-amber-50 hover:bg-amber-100 rounded-lg">项目中心</a>'
              + '<a href="/admin/projects/'+p.id+'/tasks" class="px-2.5 py-1 text-xs text-indigo-700 bg-indigo-50 hover:bg-indigo-100 rounded-lg">任务</a>'
              + '<a href="/admin/projects/'+p.id+'#tab=channels" class="px-2.5 py-1 text-xs text-emerald-700 bg-emerald-50 hover:bg-emerald-100 rounded-lg">版本</a>'
              + '<a href="/admin/projects/'+p.id+'/build" class="px-2.5 py-1 text-xs text-orange-700 bg-orange-50 hover:bg-orange-100 rounded-lg">构建</a>'
              + '<a href="/download-center?project='+encodeURIComponent(p.id)+'" class="px-2.5 py-1 text-xs text-teal-700 bg-teal-50 hover:bg-teal-100 rounded-lg">下载</a>'
              + '<span class="text-slate-400 text-xs">仅查看</span>';
        return '<tr class="'+rowClass+' hover:bg-indigo-50/40"><td class="px-6 py-3.5">'+iconHtml+'</td><td class="px-6 py-3.5 font-medium text-slate-900">'+p.id+'</td><td class="px-6 py-3.5 text-slate-800">'+p.name+statusBadge+'</td><td class="px-6 py-3.5 text-xs text-slate-500">'+(p.name_en||'-')+'</td><td class="px-6 py-3.5 text-xs text-slate-500">'+phaseLabel+'</td><td class="px-6 py-3.5 text-xs text-slate-500 max-w-[140px] truncate" title="'+((p.intro||'')+'').replace(/"/g,'&quot;')+'">'+introSnip+'</td><td class="px-6 py-3.5 text-xs text-slate-500">'+(p.created_by||'-')+'</td><td class="px-6 py-3.5 text-xs text-slate-500">'+dateStr+'</td><td class="px-6 py-3.5 text-xs text-slate-600">'+(p.task_count||0)+'</td><td class="px-6 py-3.5 text-xs text-slate-600">'+p.apk_count+'</td><td class="px-6 py-3.5 text-xs text-slate-600">'+(p.download_count||0)+'</td><td class="px-6 py-3.5"><span class="inline-flex flex-wrap gap-1.5">'+actions+'</span></td></tr>';
    }).join('');
}
...
function ensureProjectDomainFields(){
    ...
    if(addViewerInput && !document.getElementById('newProjectPlayerPublicUrl')){
        ...
        addRow.innerHTML =
            '<div><label class="block text-xs font-medium text-slate-500 mb-1">玩家官网域名</label><input type="text" id="newProjectPlayerPublicUrl" placeholder="https://game.example.com" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500"></div>'
            + '<div><label class="block text-xs font-medium text-slate-500 mb-1">论坛域名</label><input type="text" id="newProjectForumPublicUrl" placeholder="https://forum.example.com" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500"></div>'
            + '<div><label class="block text-xs font-medium text-slate-500 mb-1">开发后台域名</label><input type="text" id="newProjectAdminPublicUrl" placeholder="https://studio.example.com" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500"></div>';
    ...
    if(editViewerInput && !document.getElementById('editProjectPlayerPublicUrl')){
        ...
        editRow.innerHTML =
            '<div><label class="block text-xs font-medium text-gray-500 mb-1">玩家官网域名</label><input type="text" id="editProjectPlayerPublicUrl" placeholder="https://game.example.com" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500"></div>'
            + '<div><label class="block text-xs font-medium text-gray-500 mb-1">论坛域名</label><input type="text" id="editProjectForumPublicUrl" placeholder="https://forum.example.com" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500"></div>'
            + '<div><label class="block text-xs font-medium text-gray-500 mb-1">开发后台域名</label><input type="text" id="editProjectAdminPublicUrl" placeholder="https://studio.example.com" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500"></div>';
...
@bp.route('/admin/projects/upload-icon', methods=['POST'])
@admin_required('projects')
def admin_projects_upload_icon():
    """上传项目图标图片，返回可访问的 URL。"""
    ...
    if ext not in ('.png', '.jpg', '.jpeg', '.gif', '.webp'):
        return jsonify({'error': '仅支持图片格式：png, jpg, gif, webp'})
    safe_name = re.sub(r'[^\w\-.]', '', f.filename)
    safe_name = safe_name[:50] or 'img'
    name = '%s_%s%s' % (secrets.token_hex(4), safe_name, ext)
    os.makedirs(PROJECT_ICONS_DIR, exist_ok=True)
    path = os.path.join(PROJECT_ICONS_DIR, name)
    try:
        f.save(path)
    except Exception as e:
        return jsonify({'error': '保存失败: %s' % str(e)})
    url = '/admin/projects/icon/%s' % name
    return jsonify({'url': url})


@bp.route('/admin/projects/icon/<filename>')
def admin_projects_serve_icon(filename):
    """提供已上传的项目图标访问。"""
    ...
    if not os.path.isfile(path) or not os.path.abspath(path).startswith(os.path.abspath(PROJECT_ICONS_DIR)):
        return '', 404
    return send_from_directory(PROJECT_ICONS_DIR, filename)


@bp.route('/admin/projects/<project_id>/tasks/upload', methods=['POST'])
@admin_required('projects')
def project_task_upload(project_id):
    """上传任务附件（图片或文件），返回可访问的 URL。"""
    ...
    if not f or not f.filename:
        return jsonify({'error': '未选择文件'})
    safe_name = re.sub(r'[^\w\-.]', '', os.path.basename(f.filename))[:80] or 'file'
    ext = os.path.splitext(safe_name)[1].lower()
    name = '%s_%s_%s' % (project_id, secrets.token_hex(4), safe_name)
    ...
    try:
        ...
        return jsonify({'error': '保存失败: %s' % str(e)})
    url = '/admin/projects/%s/tasks/file/%s' % (project_id, name)
    return jsonify({'url': url, 'name': safe_name})


@bp.route('/admin/projects/<project_id>/tasks/file/<filename>')
@admin_required('projects')
def project_task_serve_file(project_id, filename):
    """提供任务附件下载。"""
    if project_id not in projects_db or not can_view_project(project_id, _current_username()):
        return '', 403
    if not filename or '..' in filename or '/' in filename or '\\' in filename:
    ...
    return send_from_directory(subdir, filename, as_attachment=True, download_name=filename.split('_', 2)[-1] if '_' in filename else filename)


@bp.route('/admin/projects/user-options')
@admin_required('projects')
def admin_projects_user_options():
    """用于项目「可查看/可编辑用户」多选的用户列表（仅普通用户，管理员默认拥有权限）。"""
    users = []
...
@bp.route('/admin/projects/validate-username')
@admin_required('projects')
def admin_projects_validate_username():
    """校验用户名是否存在（用于添加参与人员）。"""
    ...
    if username not in users_db:
        return jsonify({'exists': False})
    if users_db[username].get('disabled'):
        return jsonify({'exists': False, 'disabled': True})
    return jsonify({'exists': True, 'username': username})


@bp.route('/admin/projects/list')
@admin_required('projects')
def admin_projects_list():
    ...
    for k, v in projects_db.items():
        if not can_view_project(k, username):
            continue
        st = v.get('status', 'active')
        if status_filter == 'active' and st == 'archived':
            continue
        if status_filter == 'archived' and st != 'archived':
            continue
        ph = v.get('phase', 'kickoff')
        tasks = (project_tasks_db.get(k) or [])
        item = {
            'id': k,
            'name': v.get('name', k),
            ...
            'icon': v.get('icon', ''),
            'phase': ph,
            'phase_label': phase_map.get(ph, ph),
            'intro': v.get('intro', '') or v.get('description', ''),
            'created_by': v.get('created_by', ''),
            'created_at': v.get('created_at', ''),
            'task_count': len(tasks),
            'apk_count': get_project_apk_count(k),
            'download_count': get_project_download_count(k),
            'can_edit': can_edit_project(k, username),
            'status': st,
            'is_template': bool(v.get('is_template')),
            'player_public_url': normalize_public_url(v.get('player_public_url')),
            'forum_public_url': normalize_public_url(v.get('forum_public_url')),
            'admin_public_url': normalize_public_url(v.get('admin_public_url')),
        }
        projects.append(item)
    projects.sort(key=lambda x: (projects_db.get(x['id']) or {}).get('order', 9999))
    return jsonify({'projects': projects})


@bp.route('/admin/channels')
@admin_required('projects')
def admin_channels_list():
    """渠道列表：用于版本渠道下拉、下载中心筛选等。"""
    chans = []
    ...
    for ch in raw:
        ...
        chans.append(item)
    chans.sort(key=lambda x: (x['order'], x['id']))
    return jsonify({'channels': chans})


@bp.route('/admin/channels/create', methods=['POST'])
@admin_required('projects')
def admin_channels_create():
    ...
    if not cid or not name:
        return jsonify({'error': '渠道 ID 与名称不能为空'})
    if not re.match(r'^[a-zA-Z0-9_-]+$', cid):
        return jsonify({'error': '渠道 ID 只能包含字母、数字、下划线和中划线'})
    existing_ids = { (c.get('id') or '').strip() for c in (channels_db if isinstance(channels_db, list) else []) }
    if cid in existing_ids:
        return jsonify({'error': '渠道 ID 已存在'})
    ch = {'id': cid, 'name': name, 'description': desc, 'order': order, 'apk_subdir': apk_subdir, 'build_param': build_param}
    if isinstance(channels_db, list):
        channels_db.append(ch)
    else:
        # 理论上不会发生，兜底
        from models.data import _default_channels  # type: ignore
        channels_db[:] = _default_channels + [ch]  # type: ignore
    save_channels()
    log_audit('channel_create', cid)
    return jsonify({'ok': True, 'channel': ch})


@bp.route('/admin/channels/update', methods=['POST'])
@admin_required('projects')
def admin_channels_update():
    """更新渠道名称/描述/排序，不允许修改 ID。"""
    data = request.get_json(force=True, silent=True) or {}
    cid = (data.get('id') or '').strip()
    if not cid:
        return jsonify({'error': '缺少渠道 ID'})
    name = (data.get('name') or '').strip()
    desc = (data.get('description') or '').strip()
    order = data.get('order')
    updated = None
    if not isinstance(channels_db, list):
        return jsonify({'error': '渠道数据异常，请联系管理员'})
    ...
    save_channels()
    log_audit('channel_update', cid)
    return jsonify({'ok': True, 'channel': updated})


@bp.route('/admin/channels/delete/<channel_id>', methods=['DELETE'])
@admin_required('projects')
def admin_channels_delete(channel_id):
    """删除渠道：若仍有版本使用该渠道则禁止删除。"""
    cid = (channel_id or '').strip()
    if not cid:
        return jsonify({'error': '缺少渠道 ID'}), 400
    # 检查是否被版本引用
    used_in = []
    for pid, vers in (project_versions_db or {}).items():
        vs = vers or []
        if any((v.get('channel') or '') == cid for v in vs if isinstance(v, dict)):
            used_in.append(pid)
    if used_in:
        return jsonify({'error': '已有版本使用该渠道，无法删除（涉及项目：%s）' % ', '.join(sorted(set(used_in)))})
    if not isinstance(channels_db, list):
        return jsonify({'error': '渠道数据异常，请联系管理员'})
    before = len(channels_db)
    channels_db[:] = [c for c in channels_db if (c.get('id') or '').strip() != cid]
    if len(channels_db) == before:
        return jsonify({'error': '渠道不存在'}), 404
    save_channels()
    log_audit('channel_delete', cid)
    return jsonify({'ok': True})
...
def admin_projects_create():
    try:
        data = request.get_json(force=True, silent=True) or {}
        project_id = (data.get('id') or data.get('project_id') or '').strip()
        name = (data.get('name') or '').strip()
        if not project_id or not name:
            return jsonify({'error': '项目ID和名称不能为空'})
        if project_id in projects_db:
            return jsonify({'error': '项目ID已存在'})
        created_by = _current_username()
        template_id = (data.get('template_id') or '').strip()
        if template_id and template_id in projects_db and can_view_project(template_id, created_by):
            tpl = projects_db[template_id]
            data = dict(data, viewers=tpl.get('viewers') or [], editors=tpl.get('editors') or [], member_roles=tpl.get('member_roles') or {}, channels=tpl.get('channels') or [])
        viewers = data.get('viewers')
        editors = data.get('editors')
        if isinstance(viewers, list):
            viewers = [str(u).strip() for u in viewers if str(u).strip() in users_db]
        elif isinstance(viewers, str):
            viewers = [u.strip() for u in viewers.replace('，', ',').split(',') if u.strip() in users_db]
        else:
            viewers = []
        if isinstance(editors, list):
            editors = [str(u).strip() for u in editors if str(u).strip() in users_db]
        elif isinstance(editors, str):
            editors = [u.strip() for u in editors.replace('，', ',').split(',') if u.strip() in users_db]
        else:
            editors = []
        member_roles = data.get('member_roles')
        if not isinstance(member_roles, dict):
            member_roles = {}
        member_roles = {str(u): (str(r) if str(r) in PROJECT_ROLES else '其他') for u, r in member_roles.items() if str(u).strip() in users_db}
        phase = (data.get('phase') or 'kickoff').strip()
        if phase not in [p[0] for p in PROJECT_PHASES]:
            phase = 'kickoff'
        projects_db[project_id] = {
            ...
            'phase': phase,
            'network_connection': (data.get('network_connection') or '').strip(),
            'git_url': (data.get('git_url') or '').strip(),
            'git_ssh_key_path': (data.get('git_ssh_key_path') or '').strip(),
            'player_public_url': normalize_public_url(data.get('player_public_url')),
            'forum_public_url': normalize_public_url(data.get('forum_public_url')),
            'admin_public_url': normalize_public_url(data.get('admin_public_url')),
            'created_at': __import__('datetime').datetime.now().isoformat(),
        ...
        save_projects()
        log_audit('create_project', project_id)
        return jsonify({'success': True})
    except Exception as e:
        return jsonify({'error': '服务器错误: %s' % str(e)}), 500


@bp.route('/admin/projects/get/<project_id>')
@admin_required('projects')
def admin_projects_get(project_id):
    if project_id not in projects_db:
        return jsonify({'error': '项目不存在'})
    if not can_view_project(project_id, _current_username()):
        return jsonify({'error': '无权限查看该项目'}), 403
    v = projects_db[project_id]
    return jsonify({
        'project': {
            'id': project_id,
...
@bp.route('/admin/projects/update', methods=['POST'])
@admin_required('projects')
def admin_projects_update():
    ...
    name = (data.get('name') or '').strip()
    if not name:
        return jsonify({'error': '名称不能为空'})
    v['name'] = name
    v['name_en'] = (data.get('name_en') or '').strip()
    v['intro'] = (data.get('intro') or '').strip()
    v['detail'] = (data.get('detail') or '').strip()
    v['icon'] = (data.get('icon') or '').strip()
    ph = (data.get('phase') or 'kickoff').strip()
    v['phase'] = ph if ph in [p[0] for p in PROJECT_PHASES] else v.get('phase', 'kickoff')
    v['network_connection'] = (data.get('network_connection') or '').strip()
    v['git_url'] = (data.get('git_url') or '').strip()
    v['git_ssh_key_path'] = (data.get('git_ssh_key_path') or '').strip()
    if 'player_public_url' in data:
        v['player_public_url'] = normalize_public_url(data.get('player_public_url'))
    if 'forum_public_url' in data:
        v['forum_public_url'] = normalize_public_url(data.get('forum_public_url'))
    if 'admin_public_url' in data:
        v['admin_public_url'] = normalize_public_url(data.get('admin_public_url'))
    viewers = data.get('viewers')
    editors = data.get('editors')
    member_roles = data.get('member_roles')
    if isinstance(viewers, list):
    ...
    if isinstance(member_roles, dict):
        v['member_roles'] = {str(u): (str(r) if str(r) in PROJECT_ROLES else '其他') for u, r in member_roles.items() if str(u).strip() in users_db}
    else:
    ...
    save_projects()
    log_audit('update_project', project_id)
    return jsonify({'success': True})


@bp.route('/admin/projects/<project_id>/channels/add', methods=['POST'])
@admin_required('projects')
def admin_projects_channels_add(project_id):
    """添加渠道到项目"""
    if project_id not in projects_db or not can_edit_project(project_id, _current_username()):
        return jsonify({'error': '无权限'}), 403
    data = request.get_json(force=True, silent=True) or {}
    cid = (data.get('channel_id') or data.get('channel') or '').strip()
    if not cid:
        return jsonify({'error': '渠道 ID 不能为空'})
    valid_ids = {(c.get('id') or '').strip() for c in (channels_db if isinstance(channels_db, list) else [])}
    if cid not in valid_ids:
        return jsonify({'error': '渠道不存在'})
    proj = projects_db[project_id]
    channels = proj.get('channels')
    if not isinstance(channels, list):
        channels = []
    if cid in channels:
        return jsonify({'error': '该渠道已在项目中'})
    channels = channels + [cid]
    proj['channels'] = channels
    save_projects()
    log_audit('project_add_channel', '%s %s' % (project_id, cid))
    return jsonify({'success': True, 'channels': channels})


@bp.route('/admin/projects/<project_id>/channels/remove', methods=['POST'])
@admin_required('projects')
def admin_projects_channels_remove(project_id):
    """从项目移除渠道。若项目未配置渠道（视为全部），则移除后保留其余渠道。"""
    if project_id not in projects_db or not can_edit_project(project_id, _current_username()):
        return jsonify({'error': '无权限'}), 403
    data = request.get_json(force=True, silent=True) or {}
    cid = (data.get('channel_id') or data.get('channel') or '').strip()
    ...
    if not isinstance(channels, list):
        channels = []
    if not channels:
        all_ids = [(c.get('id') or '').strip() for c in (channels_db if isinstance(channels_db, list) else []) if (c.get('id') or '').strip()]
        channels = [c for c in all_ids if c != cid]
    else:
        channels = [c for c in channels if str(c).strip() != cid]
    proj['channels'] = channels
    save_projects()
    log_audit('project_remove_channel', '%s %s' % (project_id, cid))
    return jsonify({'success': True, 'channels': channels})


@bp.route('/admin/projects/<project_id>/archive', methods=['POST'])
@admin_required('projects')
def admin_projects_archive(project_id):
    if project_id not in projects_db:
        return jsonify({'error': '项目不存在'})
    if not can_edit_project(project_id, _current_username()):
    ...
    projects_db[project_id]['status'] = 'archived' if archive else 'active'
    save_projects()
    log_audit('project_archive' if archive else 'project_unarchive', project_id)
    return jsonify({'success': True})


@bp.route('/admin/projects/delete/<project_id>', methods=['DELETE'])
@admin_required('projects')
def admin_projects_delete(project_id):
    ...
    # 高危操作需审批：若开启，则须先有已通过的 delete_project 审批
    if str(get_system_config('REQUIRE_APPROVAL_FOR_DELETE') or '').lower() in ('true', '1', 'yes'):
        if not get_approved_approval('delete_project', project_id):
            return jsonify({'error': '删除项目需先提交审批并通过。请至 审批管理 发起「删除项目」申请，目标 ID 填写：' + project_id}), 403
    # 删除项目信息
    ...
    log_audit('delete_project', project_id)
    return jsonify({'success': True})
...
def _project_detail_html(project_id, proj, can_edit, task_stats, recent_tasks, apk_count, project_apk_files=None):
    ...
    for t in recent_tasks[:8]:
        ...
        title = (t.get('title') or '未命名')[:50]
        assignee = t.get('current_assignee') or t.get('current_role') or '-'
        tid = t.get('id', '')
            ...
            <div class="flex items-start justify-between flex-wrap gap-4">
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <h2 class="text-2xl font-semibold text-white mt-1">''' + html.escape(project_name) + '''</h2>
                ...
                <div class="flex items-center gap-2 flex-shrink-0">
                    <a href="/admin/projects/''' + project_id + '''/tasks" class="inline-flex items-center px-4 py-2.5 rounded-xl bg-indigo-500 text-white text-sm font-medium shadow-sm hover:bg-indigo-400 transition"><i class="fas fa-tasks mr-1.5"></i> 任务</a>
                    <a href="/admin/projects/''' + project_id + '''/tasks/stats" class="inline-flex items-center px-4 py-2.5 rounded-xl bg-white/10 text-slate-100 text-sm font-medium hover:bg-white/20 transition"><i class="fas fa-chart-bar mr-1.5"></i> 统计</a>
                    <a href="/docs?module=projects" class="inline-flex items-center px-3 py-2 rounded-xl bg-white/10 text-slate-200 text-sm font-medium hover:bg-white/20 transition" title="查看项目中心相关文档"><i class="fas fa-question-circle mr-1"></i> 文档</a>
                </div>
            </div>
        </div>
        <div class="p-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="bg-gradient-to-br from-amber-50 to-orange-50 rounded-2xl border border-amber-100 p-4">
                        <h3 class="text-sm font-semibold text-amber-900 mb-2">快捷操作</h3>
                        <div class="space-y-2">
                            <a href="/admin/projects/''' + project_id + '''/tasks" class="flex items-center gap-2 py-2 px-3 rounded-xl bg-white/80 hover:bg-white text-slate-800 text-sm transition"><i class="fas fa-plus text-indigo-500"></i> 新建任务</a>
                            <a href="/admin/projects/''' + project_id + '''#tab=channels" class="flex items-center gap-2 py-2 px-3 rounded-xl bg-white/80 hover:bg-white text-slate-800 text-sm transition"><i class="fas fa-layer-group text-amber-500"></i> 渠道与版本</a>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 overflow-hidden">
    <div class="p-6">
        <div class="flex border-b border-slate-200 mb-6 gap-2 items-center">
            <button type="button" onclick="switchTab('overview')" id="tabOverview" class="tab-btn px-4 py-2 text-sm font-medium text-amber-700 border-b-2 border-amber-600 -mb-px">仪表盘</button>
            <button type="button" onclick="switchTab('channels')" id="tabChannels" class="tab-btn px-4 py-2 text-sm font-medium text-slate-500 hover:text-slate-800">渠道</button>
        ...
        <script>window.switchTab=function(n){var d=document;d.querySelectorAll('.tab-btn').forEach(function(b){b.classList.remove('text-amber-600','border-b-2','border-amber-600');b.classList.add('text-gray-500');});d.querySelectorAll('.tab-panel').forEach(function(p){p.classList.add('hidden');});var btn=d.getElementById('tab'+n.charAt(0).toUpperCase()+n.slice(1)),pnl=d.getElementById('panel'+n.charAt(0).toUpperCase()+n.slice(1));if(btn){btn.classList.remove('text-gray-500');btn.classList.add('text-amber-600','border-b-2','border-amber-600');}if(pnl){pnl.classList.remove('hidden');}if(n==='channels'&&typeof renderChannelsView==='function')renderChannelsView();if(history.replaceState)history.replaceState(null,'','#tab='+n);};</script>
        ...
        <div id="panelChannels" class="tab-panel hidden">
            <div class="mb-6 p-4 rounded-2xl bg-gradient-to-br from-slate-50 to-indigo-50/30 border border-slate-100">
                ...
                <div class="flex flex-wrap gap-2 items-center">
                    ...
                    ''' + ('<div class="relative"><button type="button" onclick="toggleAddChannelDropdown()" class="inline-flex items-center px-4 py-2 rounded-xl border-2 border-dashed border-indigo-200 text-indigo-600 text-sm hover:bg-indigo-50 hover:border-indigo-300 transition"><i class="fas fa-plus mr-1.5"></i> 添加渠道</button><div id="addChannelDropdown" class="hidden absolute left-0 top-full mt-1 bg-white border border-slate-200 rounded-xl shadow-lg py-2 z-20 min-w-[160px]"></div></div>' if can_edit else '') + '''
                </div>
            </div>
...
<!-- 版本编辑弹窗 -->
<div id="versionModal" class="hidden fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onclick="if(event.target===this) closeVersionModal()">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto border border-slate-100" onclick="event.stopPropagation()">
        <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/70"><h3 class="font-semibold text-slate-900" id="versionModalTitle">新建版本</h3></div>
        <div class="px-6 py-4 space-y-3">
            <input type="hidden" id="versionEditId" value="">
            <div class="grid grid-cols-2 gap-3">
                <div><label class="block text-xs text-slate-600 mb-1">渠道</label><select id="versionChannel" onchange="suggestVersionApkPath()" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500">''' + ''.join('<option value="%s">%s</option>' % (k, v) for k, v in _get_version_channels(project_id)) + '''</select></div>
                <div><label class="block text-xs text-slate-600 mb-1">阶段</label><select id="versionStage" onchange="suggestVersionApkPath()" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500">''' + ''.join('<option value="%s">%s</option>' % (k, v) for k, v in VERSION_STAGES) + '''</select></div>
                <div><label class="block text-xs text-slate-600 mb-1">平台</label><select id="versionPlatform" onchange="suggestVersionApkPath(); syncVersionPlatformFields()" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500"><option value="android">Android</option><option value="ios">iOS</option></select></div>
                <div><label class="block text-xs text-slate-600 mb-1">版本名</label><input type="text" id="versionName" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500" placeholder="如 1.0.0"></div>
                <div><label class="block text-xs text-slate-600 mb-1">Version Code</label><input type="text" id="versionCode" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500" placeholder="如 100"></div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div><label class="block text-xs text-slate-600 mb-1">平台发布方式</label><select id="versionDistributionMethod" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500"><option value="direct">直接下载</option><option value="enterprise">企业分发</option><option value="store">应用商店</option><option value="testflight">TestFlight</option><option value="internal">内部包体</option></select></div>
                <div id="androidVersionFields" class="grid grid-cols-2 gap-3">
                    <div><label class="block text-xs text-slate-600 mb-1">Android 包名</label><input type="text" id="versionPackageName" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500" placeholder="com.example.app"></div>
                    <div><label class="block text-xs text-slate-600 mb-1">最低 SDK</label><input type="text" id="versionMinSdk" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500" placeholder="24"></div>
                </div>
                <div id="iosVersionFields" class="hidden grid grid-cols-2 gap-3 md:col-span-1">
                    <div><label class="block text-xs text-slate-600 mb-1">Bundle ID</label><input type="text" id="versionBundleId" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500" placeholder="com.example.iosapp"></div>
                    <div><label class="block text-xs text-slate-600 mb-1">最低 iOS</label><input type="text" id="versionMinIosVersion" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500" placeholder="16.4"></div>
                </div>
            </div>
            <div><label class="block text-xs text-slate-600 mb-1">安装包/下载路径</label><input type="text" id="versionApkPath" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500" placeholder="安装包输出或下载路径"></div>
            <div><label class="block text-xs text-slate-600 mb-1">资源路径</label><input type="text" id="versionResourcePath" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500" placeholder="资源目录"></div>
            <div><label class="block text-xs text-slate-600 mb-1">配置路径</label><input type="text" id="versionConfigPath" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500" placeholder="配置目录"></div>
            <div><label class="block text-xs text-slate-600 mb-1">Jenkins Job ID</label><input type="text" id="versionJenkinsJob" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500" placeholder="可选"></div>
            <div><label class="block text-xs text-slate-600 mb-1">更新说明（关联本版本，下载页展示）</label><textarea id="versionChangelog" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500" rows="2" placeholder="可选，与本版本匹配的 APK 将共用此说明"></textarea></div>
            <div><label class="block text-xs text-slate-600 mb-1"><input type="checkbox" id="versionChangelogRecommended" class="mr-1.5"> 推荐版本</label></div>
            <div><label class="block text-xs text-slate-600 mb-1">备注</label><textarea id="versionNotes" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500" rows="2"></textarea></div>
        </div>
        <div class="px-6 py-4 border-t border-slate-100 bg-slate-50/60 flex justify-end gap-2">
            ...
            <button type="button" onclick="saveVersion()" class="px-4 py-2 bg-amber-600 text-white rounded-lg text-sm font-medium hover:bg-amber-700 shadow-sm">保存</button>
        </div>
    </div>
</div>
...
window.switchTab=function switchTab(name){
    ...
    if(name==='channels') renderChannelsView();
    if(typeof history!=='undefined'&&history.replaceState) history.replaceState(null,'','#tab='+name);
};
...
function openVersionDownloadsModal(vid){
    ...
    fetch('/api/projects/'+encodeURIComponent(PROJECT_ID)+'/versions/'+encodeURIComponent(vid)+'/downloads', {credentials:'same-origin'}).then(function(r){ return r.json(); }).then(function(d){
        ...
        files.forEach(function(f){
            ...
            html+='<div class="flex items-center justify-between py-3 border-b border-slate-100 last:border-0"><div><p class="font-medium text-slate-800">'+_esc(f.name)+'</p><p class="text-xs text-slate-500">'+f.size_mb+' MB · 下载 '+f.downloads+' 次</p></div><div class="flex items-center gap-3"><a href="'+_esc(f.url)+'" class="px-3 py-1.5 rounded-lg bg-indigo-600 text-white text-xs hover:bg-indigo-700">下载</a><button type="button" data-qr-url="'+_esc(urlPath)+'" onclick="window._showQR(this.getAttribute(String.fromCharCode(100,97,116,97,45,113,114,45,117,114,108)))" class="px-3 py-1.5 rounded-lg border border-slate-200 text-slate-600 text-xs hover:bg-slate-50">二维码</button></div></div>';
        });
        html+='</div>';
        list.innerHTML=html;
...
(function(){
    ...
    if(h&&h.indexOf('tab=')>=0){ var m=h.match(/tab=([a-z]+)/); if(m) switchTab(m[1]); }
...
function renderChannelsView(){
    ...
    for(var i=0;i<chs.length;i++){
        var ch = chs[i]; var cid = ch.id||''; var cname = ch.name||cid; var blockId = 'ch-block-' + i;
        html += '<div class="channel-card bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden" data-block-id="'+blockId+'">';
        html += '<div class="px-5 py-3.5 bg-gradient-to-r from-slate-50 to-white border-b border-slate-100 flex flex-wrap items-center justify-between gap-3">';
        html += '<h4 class="font-semibold text-slate-800 flex items-center gap-2"><span class="w-8 h-8 rounded-lg bg-indigo-100 text-indigo-600 flex items-center justify-center text-sm"><i class="fas fa-mobile-alt"></i></span>'+_esc(cname)+'</h4>';
        html += '<div class="flex rounded-lg bg-slate-100/80 p-0.5">';
        for(var s=0;s<stages.length;s++){
            var st=stages[s]; var sid=st.id||'dev'; var sname=st.name||'开发';
            var isFirst = s===0;
            html += '<button type="button" class="channel-stage-tab px-3.5 py-1.5 text-xs font-medium rounded-md transition '+(isFirst?'bg-white text-slate-800 shadow-sm':'text-slate-600 hover:text-slate-800')+'" data-block="'+blockId+'" data-stage="'+_esc(sid)+'">'+_esc(sname)+'</button>';
        }
        html += '</div></div>';
        for(var s=0;s<stages.length;s++){
            var st=stages[s]; var sid=st.id||'dev'; var sname=st.name||'开发';
            var vers = allVersions.filter(function(v){ return ((v.channel||'').toLowerCase()===(cid||'').toLowerCase()) && ((v.stage||'dev')===(sid||'dev')); });
            var panelHidden = s>0 ? ' hidden' : '';
            html += '<div class="channel-stage-panel'+panelHidden+'" data-block="'+blockId+'" data-stage="'+_esc(sid)+'">';
            html += '<div class="px-5 py-3 flex justify-between items-center border-b border-slate-50">';
            html += '<span class="text-xs text-slate-500">'+_esc(sname)+' 阶段 · '+vers.length+' 个版本</span>';
            html += '<span class="inline-flex gap-2">';
            if(CAN_EDIT) html += '<button type="button" data-channel-id="'+_esc(cid)+'" data-stage="'+_esc(sid)+'" class="ch-open-version-btn inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-amber-500 text-white text-xs font-medium hover:bg-amber-600 shadow-sm transition"><i class="fas fa-plus text-[10px]"></i>新建版本</button>';
            html += '</span></div><div class="p-5">';
            if(vers.length===0){
                html += '<div class="py-10 text-center rounded-xl bg-slate-50/60 border border-dashed border-slate-200"><i class="fas fa-cube text-3xl text-slate-200 mb-2"></i><p class="text-slate-500 text-sm">暂无版本</p><p class="text-slate-400 text-xs mt-0.5">点击「新建版本」添加</p></div>';
            } else {
html += '<div class="overflow-x-auto rounded-xl border border-slate-100"><table class="w-full text-sm"><thead><tr class="bg-slate-50/80 text-left text-slate-500"><th class="px-4 py-3 text-xs font-medium w-8"></th><th class="px-4 py-3 text-xs font-medium">版本号</th><th class="px-4 py-3 text-xs font-medium">平台</th><th class="px-4 py-3 text-xs font-medium">Version Code</th><th class="px-4 py-3 text-xs font-medium">安装包状态</th><th class="px-4 py-3 text-xs font-medium">下载量</th><th class="px-4 py-3 text-xs font-medium">下载路径</th>'+(CAN_EDIT?'<th class="px-4 py-3 text-xs font-medium w-36">操作</th>':'')+'</tr></thead><tbody>';
                for(var j=0;j<vers.length;j++){
                    var v=vers[j]; var vid=v.id||''; var rowCls = j%2===0 ? 'bg-white' : 'bg-slate-50/40';
                    var actions = CAN_EDIT ? '<span class="inline-flex gap-2"><a href="/admin/projects/'+PROJECT_ID+'/versions/'+_esc(vid)+'/workflow" class="version-build-link inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-indigo-500 text-white text-xs font-medium hover:bg-indigo-600"><i class="fas fa-cogs text-[10px]"></i>构建</a><button type="button" data-version-id="'+_esc(vid)+'" class="version-downloads-btn text-emerald-600 hover:text-emerald-700 text-xs">下载列表</button><button type="button" data-version-id="'+_esc(vid)+'" class="version-edit-btn text-indigo-600 hover:text-indigo-700 text-xs">编辑</button><button type="button" data-version-id="'+_esc(vid)+'" class="version-delete-btn text-red-500 hover:text-red-600 text-xs">删除</button></span>' : '<span class="inline-flex gap-2"><a href="/admin/projects/'+PROJECT_ID+'/versions/'+_esc(vid)+'/workflow" class="version-build-link inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-indigo-500 text-white text-xs font-medium hover:bg-indigo-600"><i class="fas fa-cogs text-[10px]"></i>构建</a><button type="button" data-version-id="'+_esc(vid)+'" class="version-downloads-btn text-emerald-600 hover:text-emerald-700 text-xs">下载列表</button></span>';
                    var apkBadge = (v.apk_status==='found') ? '<span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs bg-emerald-50 text-emerald-700 border border-emerald-100"><i class="fas fa-check-circle text-[10px]"></i>已落盘</span>' : ((v.apk_status==='not_found') ? '<span class="px-2 py-0.5 rounded-md text-xs bg-slate-100 text-slate-500">未找到</span>' : '—');
                    var recBadge = (v.recommended) ? ' <span class="px-1.5 py-0.5 rounded text-[10px] bg-amber-100 text-amber-700">推荐</span>' : '';
                    var platformDetails = v.platform==='ios'
                        ? '<span>Bundle ID: '+_esc(v.bundle_id||'-')+'</span><span>最低 iOS: '+_esc(v.min_ios_version||'-')+'</span>'
                        : '<span>Android 包名: '+_esc(v.package_name||'-')+'</span><span>最低 SDK: '+_esc(v.min_sdk||'-')+'</span>';
                    var paramsHtml = '<div class="grid grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-1 text-xs text-slate-600"><span>发布方式: '+_esc(v.distribution_method||'-')+'</span>'+platformDetails+'<span>Jenkins: '+_esc(v.jenkins_job_id||'-')+'</span><span>备注: '+_esc((v.notes||'-').slice(0,50))+'</span><span>资源路径: '+_esc(v.resource_path||'-')+'</span><span>配置路径: '+_esc(v.config_path||'-')+'</span></div>';
                    html += '<tr class="'+rowCls+' border-b border-slate-50 hover:bg-indigo-50/30 transition"><td class="px-2 py-1"><button type="button" class="version-fold-btn text-slate-400 hover:text-slate-600 p-1" data-version-id="'+_esc(vid)+'" title="展开/收起"><i class="fas fa-chevron-down text-[10px]"></i></button></td><td class="px-4 py-2.5 font-medium text-slate-800">'+_esc(v.version_name||'-')+recBadge+'</td><td class="px-4 py-2.5 text-slate-600 text-xs">'+_esc(v.platform_label||v.platform||'-')+'</td><td class="px-4 py-2.5 text-slate-600 font-mono text-xs">'+_esc(v.version_code||'-')+'</td><td class="px-4 py-2.5">'+apkBadge+'</td><td class="px-4 py-2.5 text-slate-600">'+(v.download_count!=null&&v.download_count>0?v.download_count:'0')+'</td><td class="px-4 py-2.5 text-slate-500 truncate max-w-[140px]" title="'+_esc(v.apk_path||'')+'">'+_esc(v.apk_path||'-')+'</td>'+(CAN_EDIT?'<td class="px-4 py-2.5">'+actions+'</td>':'')+'</tr>';
                    html += '<tr class="version-params-row hidden '+rowCls+' border-b border-slate-50" data-version-id="'+_esc(vid)+'"><td colspan="'+(CAN_EDIT?8:7)+'" class="px-4 py-2 pb-3 bg-slate-50/50">'+paramsHtml+'</td></tr>';
                }
...
document.getElementById("channelsVersionsWrap")&&document.getElementById("channelsVersionsWrap").addEventListener("click",function(e){
    var t=e.target.closest(".ch-open-version-btn");
    if(t){openVersionModal(t.getAttribute("data-channel-id")||"",t.getAttribute("data-stage")||"dev");return;}
    t=e.target.closest(".version-fold-btn");if(t){var vid=t.getAttribute("data-version-id"); var row=document.querySelector('.version-params-row[data-version-id="'+vid+'"]'); if(row){ row.classList.toggle('hidden'); t.querySelector('i').className=row.classList.contains('hidden')?'fas fa-chevron-down text-[10px]':'fas fa-chevron-up text-[10px]'; } return;}
    t=e.target.closest(".version-downloads-btn");if(t){openVersionDownloadsModal(t.getAttribute("data-version-id"));return;}
    t=e.target.closest(".version-edit-btn");if(t){editVersion(t.getAttribute("data-version-id"));return;}
    t=e.target.closest(".version-delete-btn");if(t){deleteVersion(t.getAttribute("data-version-id"));}
});
function toggleAddChannelDropdown(){
    var dd = document.getElementById('addChannelDropdown');
    if(!dd) return;
    if(dd.classList.contains('hidden')){ dd.classList.remove('hidden'); dd.innerHTML = ''; var cur = (PROJECT_CHANNELS||[]).map(function(c){ return (c.id||'').toLowerCase(); }); var opts = ALL_CHANNELS||[]; for(var i=0;i<opts.length;i++){ var o=opts[i]; if(cur.indexOf((o.id||'').toLowerCase())>=0) continue; dd.innerHTML += '<button type="button" data-channel-id='+JSON.stringify(o.id||'')+' class="add-channel-btn w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-100">'+_esc(o.name||o.id)+'</button>'; } if(dd.innerHTML==='') dd.innerHTML='<p class="px-4 py-2 text-slate-500 text-sm">无更多渠道可添加</p>'; }
...
function openVersionModal(preselectedChannel, preselectedStage){ document.getElementById('versionModalTitle').textContent='新建版本'; document.getElementById('versionEditId').value=''; var defCh = preselectedChannel || ((PROJECT_CHANNELS&&PROJECT_CHANNELS[0]) ? PROJECT_CHANNELS[0].id : 'dev'); var defSt = preselectedStage || 'dev'; ['versionChannel','versionStage','versionPlatform','versionName','versionCode','versionApkPath','versionResourcePath','versionConfigPath','versionJenkinsJob','versionChangelog','versionNotes','versionPackageName','versionMinSdk','versionBundleId','versionMinIosVersion'].forEach(function(id){ var e=document.getElementById(id); if(e) e.value=e.type==='textarea' ? '' : (id==='versionChannel' ? defCh : (id==='versionStage' ? defSt : (id==='versionPlatform' ? 'android' : ''))); }); var distributionEl=document.getElementById('versionDistributionMethod'); if(distributionEl) distributionEl.value='direct'; var rec=document.getElementById('versionChangelogRecommended'); if(rec) rec.checked=false; syncVersionPlatformFields(); suggestVersionApkPath(); document.getElementById('versionModal').classList.remove('hidden'); }
function closeVersionModal(){ document.getElementById('versionModal').classList.add('hidden'); }

function editVersion(id){ var v=allVersions.find(function(x){ return (x.id||'')===id; }); if(!v) return; document.getElementById('versionModalTitle').textContent='编辑版本'; document.getElementById('versionEditId').value=v.id||''; document.getElementById('versionChannel').value=v.channel||'dev'; var stageEl=document.getElementById('versionStage'); if(stageEl) stageEl.value=v.stage||'dev'; var platformEl=document.getElementById('versionPlatform'); if(platformEl) platformEl.value=v.platform||'android'; document.getElementById('versionName').value=v.version_name||''; document.getElementById('versionCode').value=v.version_code||''; document.getElementById('versionApkPath').value=v.apk_path||''; document.getElementById('versionResourcePath').value=v.resource_path||''; document.getElementById('versionConfigPath').value=v.config_path||''; document.getElementById('versionJenkinsJob').value=v.jenkins_job_id||''; document.getElementById('versionChangelog').value=v.changelog_text||''; document.getElementById('versionChangelogRecommended').checked=!!v.changelog_recommended; document.getElementById('versionNotes').value=v.notes||''; var distributionEl=document.getElementById('versionDistributionMethod'); if(distributionEl) distributionEl.value=v.distribution_method||''; var packageNameEl=document.getElementById('versionPackageName'); if(packageNameEl) packageNameEl.value=v.package_name||''; var minSdkEl=document.getElementById('versionMinSdk'); if(minSdkEl) minSdkEl.value=v.min_sdk||''; var bundleIdEl=document.getElementById('versionBundleId'); if(bundleIdEl) bundleIdEl.value=v.bundle_id||''; var minIosEl=document.getElementById('versionMinIosVersion'); if(minIosEl) minIosEl.value=v.min_ios_version||''; syncVersionPlatformFields(); suggestVersionApkPath(); document.getElementById('versionModal').classList.remove('hidden'); }
...
def project_detail_page(project_id):
    ...
    content = _project_detail_html(project_id, proj, can_edit, task_stats, recent_tasks, apk_count, project_apk_files)
    return _admin_layout(content, '项目 - %s' % proj.get('name', project_id), back_href='/admin/projects')


@bp.route('/api/projects/<project_id>/download-stats')
@login_required
def project_download_stats(project_id):
    """本项目下载量与最近7日趋势，项目成员可访问"""
    if project_id not in projects_db or not can_view_project(project_id, _current_username()):
        return jsonify({'error': '无权限'}), 403
    total = get_project_download_count(project_id)
    ...
    return jsonify({'total': total, 'trend_7d': trend, 'sum_7d': sum(t['count'] for t in trend)})


@bp.route('/admin/projects/<project_id>/versions/list')
@admin_required('projects')
def project_versions_list(project_id):
    if project_id not in projects_db or not can_view_project(project_id, _current_username()):
        return jsonify({'error': '无权限'}), 403
    versions = (project_versions_db.get(project_id) or [])
    if not isinstance(versions, list):
...
@bp.route('/api/projects/<project_id>/versions/<version_id>/downloads')
@login_required
def api_version_downloads(project_id, version_id):
    """获取某版本可下载的 APK 列表（含下载链接），用于下载列表弹窗。"""
    if project_id not in projects_db or not can_view_project(project_id, _current_username()):
        return jsonify({'error': '无权限'}), 403
    versions = (project_versions_db.get(project_id) or [])
...
@bp.route('/admin/projects/<project_id>/versions/create', methods=['POST'])
@admin_required('projects')
def project_versions_create(project_id):
    if project_id not in projects_db or not can_edit_project(project_id, _current_username()):
        return jsonify({'error': '无权限'}), 403
    data = request.get_json(silent=True) or {}
    ...
    if ch_text or ch_rec:
        changelog_db['version:' + project_id + ':' + vid] = {'text': ch_text, 'recommended': ch_rec}
        save_changelog()
    log_audit('create_project_version', '%s %s' % (project_id, vid))
    v_out = {**v, 'platform_label': get_platform_label(platform), 'apk_status': 'found' if version_has_apk(project_id, v) else 'not_found', 'download_count': get_version_download_count(project_id, v), 'recommended': version_is_recommended(project_id, v), 'changelog_text': ch_text, 'changelog_recommended': ch_rec}
    return jsonify({'version': v_out})


@bp.route('/admin/projects/<project_id>/versions/update', methods=['POST'])
@admin_required('projects')
def project_versions_update(project_id):
    if project_id not in projects_db or not can_edit_project(project_id, _current_username()):
        return jsonify({'error': '无权限'}), 403
    data = request.get_json(silent=True) or {}
    vid = (data.get('id') or '').strip()
    if not vid:
        return jsonify({'error': '缺少版本 ID'}), 400
    versions = (project_versions_db.get(project_id) or [])
    if not isinstance(versions, list):
        versions = []
    ...
    if ch_text or ch_rec:
        changelog_db[vkey] = {'text': ch_text, 'recommended': ch_rec}
    else:
        changelog_db.pop(vkey, None)
    save_changelog()
    log_audit('update_project_version', '%s %s' % (project_id, vid))
    v_out = {**versions[idx], 'platform_label': get_platform_label(platform), 'apk_status': 'found' if version_has_apk(project_id, versions[idx]) else 'not_found', 'download_count': get_version_download_count(project_id, versions[idx]), 'recommended': version_is_recommended(project_id, versions[idx]), 'changelog_text': ch_text, 'changelog_recommended': ch_rec}
    return jsonify({'version': v_out})


@bp.route('/admin/projects/<project_id>/versions/delete/<version_id>', methods=['DELETE'])
@admin_required('projects')
def project_versions_delete(project_id, version_id):
    if project_id not in projects_db or not can_edit_project(project_id, _current_username()):
        return jsonify({'error': '无权限'}), 403
    # 高危操作需审批：target_id 为 project_id:version_id
    if str(get_system_config('REQUIRE_APPROVAL_FOR_DELETE') or '').lower() in ('true', '1', 'yes'):
        target_id = '%s:%s' % (project_id, version_id)
        if not get_approved_approval('delete_version', target_id):
            return jsonify({'error': '删除版本需先提交审批并通过。请至 审批管理 发起「删除版本」申请，目标 ID 填写：' + target_id}), 403
    versions = (project_versions_db.get(project_id) or [])
    if not isinstance(versions, list):
        versions = []
    ...
    project_versions_db[project_id] = versions
    save_project_versions()
    log_audit('delete_project_version', '%s %s' % (project_id, version_id))
    return jsonify({'success': True})
...
# ---------- 项目任务 ----------
@bp.route('/admin/my-tasks/set-plan', methods=['POST'])
@admin_required('projects')
def my_tasks_set_plan():
    """设置任务的用户规划类型：today_todo, today_done, tomorrow_plan, backlog"""
    username = _current_username()
    data = request.get_json() or {}
    ...
    from models.data import set_task_plan, project_tasks_db
    ...
    set_task_plan(username, project_id, task_id, plan_type)
    return jsonify({'ok': True})
...
<div class="bg-white rounded-2xl shadow-md border border-gray-200 overflow-hidden">
    <div class="px-6 py-6 border-b border-gray-100 bg-gradient-to-r from-slate-50 to-white">
        <div class="flex justify-between items-center flex-wrap gap-3">
            ...
            <a href="/admin" class="inline-flex items-center px-4 py-2 rounded-lg text-sm font-medium text-indigo-600 bg-indigo-50 hover:bg-indigo-100 transition">← 返回管理中心</a>
        </div>
        ''' + reminder + '''
        ''' + quick_tabs + '''
        ''' + plan_tabs + '''
...
<div id="myTaskEditModal" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center z-[70] p-4 overflow-y-auto" onclick="if(event.target===this) document.getElementById('myTaskEditModal').classList.add('hidden')">
    <div id="myTaskEditModalContent" class="bg-white rounded-2xl shadow-2xl max-w-lg w-full my-4 p-6 max-h-[90vh] overflow-y-auto" onclick="event.stopPropagation()">
        ...
        <div class="space-y-3 text-sm">
            <div><label class="block text-gray-600 mb-1 font-medium">任务名称</label><input type="text" id="myTaskEditTitle" class="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"></div>
            <div><label class="block text-gray-600 mb-1 font-medium">具体内容</label><textarea id="myTaskEditContent" rows="3" class="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500"></textarea></div>
            <div class="grid grid-cols-2 gap-3"><div><label class="block text-gray-600 mb-1 font-medium">开始时间</label><input type="date" id="myTaskEditStart" class="w-full px-3 py-2 border border-gray-200 rounded-lg"></div><div><label class="block text-gray-600 mb-1 font-medium">完成时间</label><input type="date" id="myTaskEditEnd" class="w-full px-3 py-2 border border-gray-200 rounded-lg"></div></div>
            ...
            <div id="myTaskEditStatusWrap" class="hidden"><div class="grid grid-cols-2 gap-3"><div><label class="block text-gray-600 mb-1 font-medium">状态</label><select id="myTaskEditStatus" class="w-full px-3 py-2 border border-gray-200 rounded-lg"></select></div><div><label class="block text-gray-600 mb-1 font-medium">当前负责人</label><select id="myTaskEditAssignee" class="w-full px-3 py-2 border border-gray-200 rounded-lg"></select></div></div></div>
            ...
            <div class="border-t pt-3 mt-3"><label class="block text-gray-600 mb-1 font-medium">评论</label><div id="myTaskEditCommentsList" class="mb-2 max-h-32 overflow-y-auto text-xs space-y-1.5 border border-gray-100 rounded p-2 bg-gray-50"></div><div class="flex gap-2"><input type="text" id="myTaskEditNewComment" placeholder="追加评论…" class="flex-1 px-3 py-2 border rounded text-sm"><button type="button" id="myTaskEditBtnComment" class="px-3 py-2 bg-blue-600 text-white rounded text-sm">发送</button></div></div>
        </div>
...
<div class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-4">
        ...
        <a href="/admin/projects/''' + project_id + '''/tasks" class="inline-flex items-center px-4 py-2 rounded-lg text-sm font-medium text-indigo-700 bg-indigo-50 hover:bg-indigo-100 transition">← 返回任务列表</a>
    </div>
...
<div id="flowLogModal" class="hidden fixed inset-0 bg-black/60 flex items-center justify-center z-[60] p-4" onclick="if(event.target===this) this.classList.add('hidden')">
    <div class="bg-white rounded-xl shadow-xl max-w-lg w-full max-h-[80vh] overflow-hidden flex flex-col" onclick="event.stopPropagation()">
        ...
        <div id="flowLogContent" class="p-4 overflow-y-auto text-sm space-y-2"></div>
        <div class="p-4 border-t"><button type="button" onclick="document.getElementById('flowLogModal').classList.add('hidden')" class="px-4 py-2 border rounded text-sm">关闭</button></div>
    </div>
</div>
<div id="commentModal" class="hidden fixed inset-0 bg-black/60 flex items-center justify-center z-[60] p-4" onclick="if(event.target===this) this.classList.add('hidden')">
    <div class="bg-white rounded-xl shadow-xl max-w-lg w-full max-h-[80vh] overflow-hidden flex flex-col" onclick="event.stopPropagation()">
        ...
        <div id="commentList" class="p-4 overflow-y-auto text-sm space-y-2 flex-1"></div>
        <div class="p-4 border-t flex gap-2">
            <input type="text" id="newCommentContent" placeholder="输入评论…" class="flex-1 px-3 py-2 border rounded text-sm">
...
<div id="statusModal" class="hidden fixed inset-0 bg-black/60 flex items-center justify-center z-[60] p-4" onclick="if(event.target===this) this.classList.add('hidden')">
    <div class="bg-white rounded-xl shadow-xl w-80 p-5" onclick="event.stopPropagation()">
        ...
        <select id="statusModalSelect" class="w-full px-3 py-2 border rounded text-sm mb-3"></select>
        <div class="flex gap-2"><button type="button" onclick="confirmStatusChange()" class="flex-1 px-4 py-2 bg-blue-600 text-white rounded text-sm">确认</button><button type="button" onclick="document.getElementById('statusModal').classList.add('hidden')" class="px-4 py-2 border rounded text-sm">取消</button></div>
    </div>
</div>
<div id="handoffModal" class="hidden fixed inset-0 bg-black/60 flex items-center justify-center z-[60] p-4" onclick="if(event.target===this) this.classList.add('hidden')">
    <div class="bg-white rounded-xl shadow-xl w-80 p-5" onclick="event.stopPropagation()">
        ...
        <select id="handoffModalSelect" class="w-full px-3 py-2 border rounded text-sm mb-3"></select>
        <div class="flex gap-2"><button type="button" onclick="confirmHandoff()" class="flex-1 px-4 py-2 bg-blue-600 text-white rounded text-sm">确认</button><button type="button" onclick="document.getElementById('handoffModal').classList.add('hidden')" class="px-4 py-2 border rounded text-sm">取消</button></div>
    </div>
</div>
<div id="editTaskModal" class="hidden fixed inset-0 bg-black/60 flex items-center justify-center z-[60] p-4 overflow-y-auto" onclick="if(event.target===this) this.classList.add('hidden')">
    <div id="editTaskModalContent" class="bg-white rounded-xl shadow-xl max-w-lg w-full my-4 p-5 max-h-[90vh] overflow-y-auto" onclick="event.stopPropagation()">
        ...
        <div class="space-y-2 text-sm">
            <div><label class="block text-gray-600 mb-1">任务名称</label><input type="text" id="editTaskTitle" class="w-full px-3 py-2 border rounded"></div>
            <div><label class="block text-gray-600 mb-1">具体内容</label><textarea id="editTaskContent" rows="3" class="w-full px-3 py-2 border rounded"></textarea></div>
            <div class="grid grid-cols-2 gap-2"><div><label class="block text-gray-600 mb-1">开始时间</label><input type="date" id="editTaskStart" class="w-full px-3 py-2 border rounded"></div><div><label class="block text-gray-600 mb-1">完成时间</label><input type="date" id="editTaskEnd" class="w-full px-3 py-2 border rounded"></div></div>
            ...
            <div id="editTaskStatusAssigneeWrap" class="hidden"><div class="grid grid-cols-2 gap-2"><div><label class="block text-gray-600 mb-1">状态</label><select id="editTaskStatus" class="w-full px-3 py-2 border rounded"></select></div><div><label class="block text-gray-600 mb-1">当前负责人</label><select id="editTaskAssignee" class="w-full px-3 py-2 border rounded"></select></div></div></div>
            ...
            <div class="border-t pt-3 mt-3"><label class="block text-gray-600 mb-1 font-medium">评论</label><div id="editTaskCommentsList" class="mb-2 max-h-32 overflow-y-auto text-xs space-y-1.5 border border-gray-100 rounded p-2 bg-gray-50"></div><div class="flex gap-2"><input type="text" id="editTaskNewComment" placeholder="追加评论…" class="flex-1 px-3 py-2 border rounded text-sm"><button type="button" id="editTaskBtnComment" class="px-3 py-2 bg-blue-600 text-white rounded text-sm">发送</button></div></div>
        </div>
        <div class="flex gap-2 mt-4"><button type="button" onclick="saveEditTask()" class="flex-1 px-4 py-2 bg-blue-600 text-white rounded text-sm">保存</button><button type="button" onclick="document.getElementById('editTaskModal').classList.add('hidden')" class="px-4 py-2 border rounded text-sm">取消</button></div>
    </div>
</div>
<script>
...
@bp.route('/admin/projects/<project_id>/tasks/list')
@admin_required('projects')
def project_tasks_list(project_id):
    ...
    stats = {'total': len(tasks), 'abandoned': 0, 'not_started': 0, 'in_progress': 0, 'pending_review': 0, 'review_passed': 0, 'review_failed': 0, 'done': 0, 'overdue': 0, 'soon_overdue': 0}
    soon_days = 3
    out = []
    for t in tasks:
        t = dict(t)
        ...
        if et and s not in ('done', 'abandoned'):
            try:
                dt = __import__('datetime').datetime.strptime(et[:10], '%Y-%m-%d')
                if dt < now:
                    stats['overdue'] += 1
                ...
                pass
    return jsonify({'tasks': out, 'my_username': username, 'participants': participants, 'stats': stats})
...
@bp.route('/admin/projects/<project_id>/tasks/create', methods=['POST'])
@admin_required('projects')
def project_task_create(project_id):
    ...
    task = {
        ...
        'status': 'not_started',
        'urgency': urgency,
        'flow_log': [{'from_user': created_by, 'to_user': assign_to_user, 'status': 'not_started', 'at': now_iso, 'by': created_by}],
        'comments': [],
        'attachments': attachments,
    }
    if project_id not in project_tasks_db:
        project_tasks_db[project_id] = []
    project_tasks_db[project_id].append(task)
    save_project_tasks()
    log_audit('create_task', '%s %s' % (project_id, task_id))
    return jsonify({'success': True, 'task_id': task_id})


@bp.route('/admin/projects/<project_id>/tasks/<task_id>/handoff', methods=['POST'])
@admin_required('projects')
def project_task_handoff(project_id, task_id):
    if project_id not in projects_db or not can_view_project(project_id, _current_username()):
        return jsonify({'error': '无权限'}), 403
    username = _current_username()
    tasks = project_tasks_db.get(project_id) or []
    ...
    if not task or cur != username:
        return jsonify({'error': '只能流转当前分配给自己的任务'}), 403
    data = request.get_json(force=True, silent=True) or {}
    to_user = (data.get('passed_to_user') or '').strip()
    proj = projects_db.get(project_id) or {}
    editors = proj.get('editors') or []
    if to_user not in editors:
        return jsonify({'error': '流转对象必须是项目参与人'}), 400
    now_iso = __import__('datetime').datetime.now().isoformat()
    ...
    save_project_tasks()
    log_audit('task_handoff', '%s %s -> %s' % (project_id, task_id, to_user))
    return jsonify({'success': True})


@bp.route('/admin/projects/<project_id>/tasks/<task_id>/update-status', methods=['POST'])
@admin_required('projects')
def project_task_update_status(project_id, task_id):
    if project_id not in projects_db or not can_view_project(project_id, _current_username()):
        return jsonify({'error': '无权限'}), 403
    username = _current_username()
    tasks = project_tasks_db.get(project_id) or []
    ...
    cur = task.get('current_assignee') or (_role_to_first_assignee(project_id, task.get('current_role')) if task else '')
    if not task or cur != username:
        return jsonify({'error': '只能修改分配给自己的任务状态'}), 403
    data = request.get_json(force=True, silent=True) or {}
    ...
    task['status'] = status
    save_project_tasks()
    log_audit('task_update_status', '%s %s -> %s' % (project_id, task_id, status))
    return jsonify({'success': True})


@bp.route('/admin/projects/<project_id>/tasks/<task_id>/comment', methods=['POST'])
@admin_required('projects')
def project_task_comment(project_id, task_id):
    if project_id not in projects_db or not can_view_project(project_id, _current_username()):
        return jsonify({'error': '无权限'}), 403
    proj = projects_db[project_id]
    editors = proj.get('editors') or []
    ...
    if username not in editors:
        return jsonify({'error': '仅项目参与人可评论'}), 403
    data = request.get_json(force=True, silent=True) or {}
    content = (data.get('content') or '').strip()
    if not content:
    ...
    save_project_tasks()
    return jsonify({'success': True, 'user': username, 'at': now_iso})


@bp.route('/admin/projects/<project_id>/tasks/<task_id>', methods=['PUT'])
@admin_required('projects')
def project_task_update(project_id, task_id):
    """更新任务。参与人可编辑标题、内容、时间、附件；仅创建人或当前负责人可编辑状态与负责人。"""
    if project_id not in projects_db or not can_view_project(project_id, _current_username()):
        return jsonify({'error': '无权限'}), 403
    username = _current_username()
    proj = projects_db[project_id]
    editors = proj.get('editors') or []
    if username not in editors:
        return jsonify({'error': '仅项目参与人可编辑任务'}), 403
    tasks = project_tasks_db.get(project_id) or []
    task = next((t for t in tasks if t.get('id') == task_id), None)
    if not task:
        return jsonify({'error': '任务不存在'}), 404
    ...
    if can_edit_status_assignee:
        ...
        if data.get('current_assignee') is not None:
            ...
            if to_user in editors and to_user != task.get('current_assignee'):
                ...
                task['flow_log'].append({'from_user': task.get('current_assignee') or username, 'to_user': to_user, 'status': task.get('status', ''), 'at': __import__('datetime').datetime.now().isoformat(), 'by': username})
                task['current_assignee'] = to_user
    save_project_tasks()
    log_audit('task_update', '%s %s' % (project_id, task_id))
    return jsonify({'success': True})


@bp.route('/admin/projects/<project_id>/tasks/<task_id>', methods=['DELETE'])
@admin_required('projects')
def project_task_delete(project_id, task_id):
    ...
    if not task:
        return jsonify({'error': '任务不存在'}), 404
    if task.get('created_by') != _current_username():
        return jsonify({'error': '仅创建者可删除任务'}), 403
    project_tasks_db[project_id] = [t for t in tasks if t.get('id') != task_id]
    save_project_tasks()
    log_audit('task_delete', '%s %s' % (project_id, task_id))
    return jsonify({'success': True})


@bp.route('/admin/projects/<project_id>/tasks/batch-delete', methods=['POST'])
@admin_required('projects')
def project_tasks_batch_delete(project_id):
    if project_id not in projects_db or not can_view_project(project_id, _current_username()):
        return jsonify({'error': '无权限'}), 403
    data = request.get_json(force=True, silent=True) or {}
    ...
    tasks = project_tasks_db.get(project_id) or []
    username = _current_username()
    kept = [t for t in tasks if t.get('id') not in ids or t.get('created_by') != username]
    removed = [t for t in tasks if t.get('id') in ids and t.get('created_by') == username]
    if len(removed) < len(ids):
        return jsonify({'error': '只能删除自己创建的任务，部分任务无法删除'}), 403
    project_tasks_db[project_id] = kept
    save_project_tasks()
    log_audit('task_batch_delete', '%s %s' % (project_id, ','.join(ids)))
    return jsonify({'success': True})
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-4 border-b bg-gray-50/50">
            ...
            <form method="get" action="/admin/audit-log" class="flex flex-wrap gap-3 items-end">
                <div><label class="block text-xs text-gray-500 mb-1">关键字</label><input type="text" name="keyword" value="{html.escape(keyword)}" placeholder="用户/操作/详情/IP" class="px-3 py-2 border border-gray-200 rounded-lg text-sm w-40"></div>
                <div><label class="block text-xs text-gray-500 mb-1">用户</label><select name="user" class="px-3 py-2 border border-gray-200 rounded-lg text-sm"><option value="">全部</option>{user_opts}</select></div>
                <div><label class="block text-xs text-gray-500 mb-1">操作类型</label><select name="action" class="px-3 py-2 border border-gray-200 rounded-lg text-sm"><option value="">全部</option>{action_opts}</select></div>
                <div><label class="block text-xs text-gray-500 mb-1">开始日期</label><input type="date" name="date_from" value="{html.escape(date_from)}" class="px-3 py-2 border border-gray-200 rounded-lg text-sm"></div>
                <div><label class="block text-xs text-gray-500 mb-1">结束日期</label><input type="date" name="date_to" value="{html.escape(date_to)}" class="px-3 py-2 border border-gray-200 rounded-lg text-sm"></div>
                <input type="hidden" name="page_size" value="{page_size}">
                <button type="submit" class="px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700">筛选</button>
...
@bp.route('/admin/audit-log/export')
@admin_required('audit_log')
...
# ---------- 通知中心 ----------
NOTIFICATION_TYPE_LABELS = {'task_assign': '任务分配', 'task_comment': '任务评论', 'approval_result': '审批结果', 'build_done': '构建完成', 'announcement': '系统公告', '': '全部'}
...
def admin_notifications_page():
    ...
    items = get_notifications_for_user(username, type_filter or None, limit=200)
    type_opts = ''.join(
        f'<option value="{t}"' + (' selected' if t == type_filter else '') + f'>{label}</option>'
        for t, label in NOTIFICATION_TYPE_LABELS.items()
    )
    project_filter_opts = ''
    rows = []
    for n in items:
        ...
        title_esc = html.escape(n.get('title') or '')
        time_esc = (n.get('created_at') or '')[:19].replace('T', ' ')
        nid = n.get('id') or ''
        if link:
        ...
        btn = ''
        if not read_at:
            btn = f'<button type="button" onclick="markRead(\'{nid}\')" class="text-xs text-gray-500 hover:text-indigo-600">标为已读</button>'
        rows.append(
            ...
            f'<td class="px-4 py-3">{title_cell}</td><td class="px-4 py-3">{btn}</td></tr>'
        )
    ...
    <!-- misplaced report filter removed
        <div>
            <label class="block text-xs text-gray-500 mb-1">按项目筛选</label>
            <select name="project_id" class="w-64 rounded-lg border px-3 py-2 text-sm">{project_filter_opts}</select>
        </div>
        <button type="submit" class="rounded-lg bg-slate-900 px-4 py-2 text-sm font-medium text-white">筛选</button>
        <a href="/admin/reports" class="rounded-lg border border-slate-200 px-4 py-2 text-sm font-medium text-slate-700">重置</a>
    -->
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 overflow-hidden">
    ...
    content = content.replace(
        '<th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">鍚嶇О</th><th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">鍒涘缓鏃堕棿</th><th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">鎿嶄綔</th>',
        '<th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">鍚嶇О</th><th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">\u9879\u76ee</th><th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">鍒涘缓鏃堕棿</th><th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">鎿嶄綔</th>',
        1
    )
    content = content.replace(
        '<th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">瀵煎嚭鏃堕棿</th><th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">妯℃澘</th><th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">鏍煎紡</th>',
        '<th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">瀵煎嚭鏃堕棿</th><th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">妯℃澘</th><th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">\u9879\u76ee</th><th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">鏍煎紡</th>',
        1
    )
    return _admin_layout(content, '通知中心')


@bp.route('/admin/notifications/<nid>/read', methods=['POST'])
@admin_required('notifications')
def admin_notification_mark_read(nid):
    username = _current_username()
    for n in notifications_db:
        if n.get('id') == nid and n.get('user') == username:
            n['read_at'] = __import__('datetime').datetime.now().isoformat()
            save_notifications()
            return jsonify({'success': True})
...
def admin_notifications_read_all():
    username = _current_username()
    for n in notifications_db:
        if n.get('user') == username and not n.get('read_at'):
            n['read_at'] = __import__('datetime').datetime.now().isoformat()
    save_notifications()
    return jsonify({'success': True})
...
def admin_approval_page():
    ...
    <section class="space-y-6">
        ...
        <form class="flex flex-wrap items-end gap-3 rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm" method="get">
            ...
            <button type="submit" class="rounded-lg bg-slate-900 px-4 py-2 text-sm font-medium text-white">\u7b5b\u9009</button>
            <a href="/admin/approval" class="rounded-lg border border-slate-200 px-4 py-2 text-sm font-medium text-slate-700">\u91cd\u7f6e</a>
        </form>
            ...
            <div id="createApprovalForm" class="hidden border-b bg-gray-50/50 p-4">
                <form onsubmit="return submitApproval(event)">
                    <div class="mb-3 grid grid-cols-1 gap-3 md:grid-cols-3">
                        <div><label class="mb-1 block text-xs text-gray-500">\u5ba1\u6279\u7c7b\u578b</label><select id="approvalType" class="w-full rounded-lg border px-3 py-2 text-sm" required>{type_opts}</select></div>
                        <div><label class="mb-1 block text-xs text-gray-500">\u6240\u5c5e\u9879\u76ee</label><select id="approvalProjectId" class="w-full rounded-lg border px-3 py-2 text-sm"><option value="">\u672a\u7ed1\u5b9a\u9879\u76ee</option>{project_opts}</select></div>
                        <div><label class="mb-1 block text-xs text-gray-500">\u5173\u8054\u5bf9\u8c61 ID\uff08\u5982\u9879\u76ee ID\u3001\u7248\u672c\u53f7\uff09</label><input type="text" id="approvalTargetId" class="w-full rounded-lg border px-3 py-2 text-sm" placeholder="\u5982 MyProject \u6216 1.0.0"></div>
                    </div>
                    <div class="mb-3"><label class="mb-1 block text-xs text-gray-500">\u7533\u8bf7\u8bf4\u660e</label><input type="text" id="approvalReason" class="w-full rounded-lg border px-3 py-2 text-sm" placeholder="\u7b80\u8981\u8bf4\u660e"></div>
                    <button type="submit" class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white">\u63d0\u4ea4</button>
                </form>
            </div>
            <div class="overflow-x-auto">
                <table class="min-w-full"><thead class="bg-gray-50"><tr>
        ...
        <div id="approveModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 p-4">
            <div class="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
                ...
                <textarea id="approveComment" class="mb-4 w-full rounded-lg border px-3 py-2 text-sm" rows="2" placeholder="\u9009\u586b"></textarea>
                <div class="flex gap-2"><button type="button" onclick="submitApprove()" class="flex-1 rounded-lg bg-indigo-600 py-2 text-sm text-white">\u786e\u5b9a</button><button type="button" onclick="document.getElementById('approveModal').classList.add('hidden')" class="rounded-lg border px-4 py-2 text-sm">\u53d6\u6d88</button></div>
            </div>
        </div>
        <meta name="csrf-token" content="{html.escape(_get_csrf_token())}">
        <script>
...
@bp.route('/admin/approval/create', methods=['POST'])
@admin_required('approval')
def admin_approval_create():
    data = request.get_json(silent=True) or {}
    atype = (data.get('type') or data.get('target_type') or '').strip()
    if atype not in [t[0] for t in APPROVAL_TYPES]:
        return jsonify({'error': '无效的审批类型'})
    target_id = (data.get('target_id') or '').strip()
    reason = (data.get('reason') or '').strip()
    project_id = resolve_project_id((data.get('project_id') or '').strip()) or ''
    username = _current_username()
    aid = create_approval(atype, username, atype, target_id, reason, project_id=project_id)
    log_audit('approval_create', '%s %s' % (atype, target_id))
    add_notification(username, 'approval_result', '审批已提交', '类型：%s，对象：%s' % (atype, target_id), '/admin/approval', aid, 'approval')
    return jsonify({'success': True, 'id': aid})


@bp.route('/admin/approval/<aid>/<action>', methods=['POST'])
@admin_required('approval')
def admin_approval_do(aid, action):
    if action not in ('approve', 'reject'):
        return jsonify({'error': '无效操作'}), 400
    # 操作级权限：仅拥有 approval.manage 的用户才能执行审批通过/驳回
    if not has_scope('approval.manage'):
        return jsonify({'error': '无权限执行审批操作'}), 403
    data = request.get_json(silent=True) or {}
    comment = (data.get('comment') or '').strip()
    username = _current_username()
    ok, err = approve_or_reject(aid, username, action, comment)
    if not ok:
        return jsonify({'error': err or '操作失败'})
    approval = None
    for a in approvals_db:
        if a.get('id') == aid:
            approval = a
            applicant = a.get('applicant', '')
            add_notification(applicant, 'approval_result', '审批已%s' % ('通过' if action == 'approve' else '驳回'), comment or ('已%s' % ('通过' if action == 'approve' else '驳回')), '/admin/approval', aid, 'approval')
            break
    if approval:
        target_status = 'published' if action == 'approve' else 'rejected'
        approval_type = approval.get('type') or ''
        target_id = approval.get('target_id') or ''
        if approval_type == 'news_publish':
            set_news_publish_state(target_id, target_status, approval.get('id'))
        elif approval_type == 'welfare_publish':
            set_welfare_publish_state(target_id, target_status, approval.get('id'))
        elif approval_type == 'forum_post_publish':
            set_forum_post_publish_state(target_id, target_status, approval.get('id'))
    log_audit('approval_%s' % action, aid)
    return jsonify({'success': True})
...
def admin_reports_page():
    ...
    report_filter_html = (
        ...
        f'<select name="project_id" class="w-64 rounded-lg border px-3 py-2 text-sm">{project_filter_opts}</select></div>'
        '<button type="submit" class="rounded-lg bg-slate-900 px-4 py-2 text-sm font-medium text-white">\u7b5b\u9009</button>'
        '<a href="/admin/reports" class="rounded-lg border border-slate-200 px-4 py-2 text-sm font-medium text-slate-700">\u91cd\u7f6e</a>'
        '</form>'
    )
    ...
    rrows = ''.join(
        f'<tr><td class="px-4 py-2 text-sm">{(r.get("exported_at") or "")[:19]}</td><td class="px-4 py-2 text-sm">{html.escape(r.get("template_name") or "")}</td><td class="px-4 py-2 text-sm">{html.escape(project_label(export_project_id(r)))}</td><td class="px-4 py-2 text-sm">{r.get("format", "csv")}</td></tr>'
        for r in recs
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 overflow-hidden">
        ...
        <div class="p-4 border-b bg-gray-50/50">
            <form id="reportTemplateForm" class="flex flex-wrap gap-3 items-end">
                <div><label class="block text-xs text-gray-500 mb-1">\u6240\u5c5e\u9879\u76ee</label><select id="tplProjectId" class="px-3 py-2 border rounded-lg text-sm min-w-[220px]">{project_create_opts}</select></div>
                <div><label class="block text-xs text-gray-500 mb-1">模板名称</label><input type="text" id="tplName" class="px-3 py-2 border rounded-lg text-sm" placeholder="如：周报汇总" required></div>
                <div><label class="block text-xs text-gray-500 mb-1">说明</label><input type="text" id="tplDesc" class="px-3 py-2 border rounded-lg text-sm" placeholder="选填"></div>
...
@bp.route('/admin/reports/templates', methods=['POST'])
@admin_required('reports')
def admin_reports_create_template():
    data = request.get_json(silent=True) or {}
    name = (data.get('name') or '').strip()
    if not name:
        return jsonify({'error': '模板名称不能为空'})
    project_id = resolve_project_id((data.get('project_id') or ((data.get('config') or {}).get('project_id')) or '').strip()) or ''
    config = data.get('config') or {}
    if not isinstance(config, dict):
        config = {}
    if project_id:
        config['project_id'] = project_id
    tid = __import__('uuid').uuid4().hex[:16]
    report_templates_db.append({
        'id': tid, 'name': name, 'description': (data.get('description') or '').strip(),
        'project_id': project_id, 'config': config, 'created_by': _current_username(),
        'created_at': __import__('datetime').datetime.now().isoformat(),
    })
    save_report_templates()
    log_audit('report_template_create', name)
    return jsonify({'success': True, 'id': tid})


@bp.route('/admin/reports/run/<template_id>')
@admin_required('reports')
def admin_reports_run(template_id):
    from flask import send_file
    import io
    import csv as csv_module
    from datetime import datetime
    from models.data import download_stats, load_download_events, extract_project_name, extract_version_from_filename
    tpl = next((t for t in report_templates_db if t.get('id') == template_id), None)
    if not tpl:
        return '模板不存在', 404
    template_project_id = resolve_project_id((((tpl.get('config') or {}).get('project_id')) or tpl.get('project_id') or '').strip()) or ''
    buf = io.StringIO()
    w = csv_module.writer(buf)
    w.writerow(['文件名', '项目', '版本', '下载次数', '统计时间'])
    for fname, count in sorted(download_stats.items(), key=lambda x: -x[1]):
        resolved_name_project = resolve_project_id(extract_project_name(fname)) or ''
        if template_project_id and resolved_name_project != template_project_id:
            continue
        w.writerow([fname, extract_project_name(fname), extract_version_from_filename(fname), count, datetime.now().strftime('%Y-%m-%d %H:%M')])
    buf.write('\n下载事件明细（最近）\n')
    w.writerow(['日期', '文件名', '来源', 'IP'])
    for e in load_download_events()[-5000:]:
        event_project_id = resolve_project_id(extract_project_name(e.get('filename', ''))) or ''
    ...
    buf = io.BytesIO(buf.getvalue().encode('utf-8-sig'))
    buf.seek(0)
    export_records_db.append({
        'user': _current_username(), 'template_id': template_id, 'template_name': tpl.get('name', ''),
        'project_id': template_project_id, 'params': tpl.get('config', {}), 'exported_at': datetime.now().isoformat(), 'format': 'csv',
    })
    save_export_records()
    log_audit('report_export', tpl.get('name', ''))
    fn = 'report_%s_%s.csv' % (template_id[:8], datetime.now().strftime('%Y%m%d_%H%M'))
    return send_file(buf, as_attachment=True, download_name=fn, mimetype='text/csv')
...
def admin_settings_page():
    ...
    for key, label, vtype, default, desc in DEFAULT_SYSTEM_KEYS:
        ...
        if vtype == 'boolean':
            ...
            inp = f'<input type="text" name="{key}" value="{html.escape(str(val))}" class="w-full max-w-md px-3 py-2 border rounded-lg text-sm" placeholder="https://...">'
        else:
            inp = f'<input type="number" name="{key}" value="{html.escape(str(val))}" class="w-32 px-3 py-2 border rounded-lg text-sm" min="1">'
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mb-6">
        ...
        <form id="settingsForm" class="p-6">
            ...
            <button type="submit" class="mt-4 px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700">保存</button>
        </form>
    </div>
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
...
def admin_settings_save():
    ...
    username = _current_username()
    # 操作级权限：修改系统与安全设置
    if not has_scope('settings.manage'):
    ...
    for key, label, vtype, default, desc in DEFAULT_SYSTEM_KEYS:
        if key in data:
            ...
            set_system_config(key, str(val).strip() if val else '', vtype, desc, username)
    log_audit('system_settings_update', '')
    return jsonify({'success': True})
