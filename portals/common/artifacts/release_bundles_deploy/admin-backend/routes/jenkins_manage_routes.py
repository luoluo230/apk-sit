        ...
        <details class="mt-4" id="detailsBuildDefaults">
            ...
            <div class="mt-3 grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                ...
                <div class="md:col-span-2">
                    <label class="block text-gray-600 mb-1">Unity 版本列表（每行：版本号 或 版本号,安装路径）</label>
                    <textarea id="newUnityVersions" rows="3" placeholder="6000.3.8f1,/Applications/Unity/Hub/Editor/6000.3.8f1/Unity.app" class="border rounded px-2 py-1.5 w-full text-xs"></textarea>
                </div>
                <div class="md:col-span-2">
...
function loadList(){
    fetch('/api/jenkins-manage/list', {credentials:'same-origin'}).then(r=>r.json()).then(d=>{
        ...
        if(!d||!d.instances){ tbody.innerHTML=''; msg.textContent='暂无实例'; return; }
        var rows=d.instances.map(function(i){
            ...
            var logBtn='<a href="/admin/jenkins/instance-log?instance_id='+encodeURIComponent(i.id)+'" target="_blank" class="text-blue-600 hover:underline">运行日志</a>';
        ...
        msg.textContent='共 '+d.instances.length+' 个实例';
...
def api_start_jenkins():
    ...
    success, instance_id, err = jm.start_jenkins(
        int(port), started_by, task_name=task_name, feishu_webhook=feishu_webhook, build_defaults=build_defaults
    )
    if success:
...
def _jenkins_edit_html(instance_id, inst):
    ...
    if bd.get('unity_versions'):
        for u in bd['unity_versions']:
            if isinstance(u, dict):
                v, p = u.get('version', ''), u.get('path', '')
                unity_lines += (v + (',' + p if p else '') + '\n')
            else:
                unity_lines += str(u) + '\n'
    ...
    <div class="bg-white rounded-lg shadow p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            ...
            <div class="md:col-span-2"><label class="block text-gray-600 mb-1">Unity 版本列表（每行：版本号 或 版本号,安装路径）</label><textarea id="editUnityVersions" rows="3" placeholder="6000.3.8f1,/Applications/Unity/Hub/Editor/6000.3.8f1/Unity.app" class="border rounded px-2 py-1.5 w-full text-xs">''' + unity_lines.replace('<', '&lt;').replace('>', '&gt;') + '''</textarea></div>
...
function collectEdit(){
    var unityText = document.getElementById('editUnityVersions').value.trim();
    var unityVersions = [];
    if(unityText) unityText.split(/\\n/).forEach(function(line){ var t=line.trim(); if(!t) return; var parts=t.split(','); unityVersions.push({version: parts[0].trim(), path: parts[1]?parts[1].trim():''}); });
...
def api_list_available_instances():
    """仅返回运行中且当前无构建任务的 Jenkins 实例，供版本构建页选择。"""
    ...
    for inst in instances:
        ...
        if not url or not bdir:
            continue
        try:
            st = jenkins_svc.fetch_jenkins_status(base_url=url, builds_dir=bdir, instance_id=inst.get('id'))
            if st.get('ok') and not st.get('building'):
