# -*- coding: utf-8 -*-
"""SSE notifications stream gate (§11.5 minimal WebSocket/SSE)."""

from __future__ import annotations

import json
import os
import sys

ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
if ROOT not in sys.path:
    sys.path.insert(0, ROOT)


def main() -> int:
    from app_new import app
    from data.repositories.user_repository import UserRepository

    repo = UserRepository()
    user = next(iter(repo.list_all().keys()), "admin")
    errors: list[str] = []
    with app.test_client() as client:
        with client.session_transaction() as sess:
            sess["user"] = user
        resp = client.get("/api/notifications/stream")
        if resp.status_code != 200:
            errors.append(f"stream status {resp.status_code}")
        elif "text/event-stream" not in (resp.content_type or ""):
            errors.append(f"unexpected content type {resp.content_type}")
    ok = not errors
    print(json.dumps({"ok": ok, "errors": errors}, ensure_ascii=False))
    return 0 if ok else 1


if __name__ == "__main__":
    raise SystemExit(main())
