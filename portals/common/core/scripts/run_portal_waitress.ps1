...
$modulePath = switch ($Mode) {
    'admin' { 'portals.intranet.wsgi:app' }
    'player' { 'portals.extranet.wsgi:app' }
    'forum' { 'portals.forum.wsgi:app' }
}

$env:PYTHONPATH = if ([string]::IsNullOrWhiteSpace($env:PYTHONPATH)) { $RootDir } else { "$RootDir;$($env:PYTHONPATH)" }

$arguments = @()
$arguments += $pythonLaunch.ArgumentPrefix
$arguments += @('-u', '-m', 'waitress', "--listen=0.0.0.0:$AppPort", $modulePath)
