(function () {
  ...
  const elements = {
    ...
    agentEditModal: document.getElementById('agentEditModal'),
  ...
  function statusMeta(status) {
    ...
    if (['ONLINE', 'READY', 'SUCCESS', 'RUNNING'].includes(value)) return { key: 'ok', label: '在线' };
    ...
    if (['STOPPED', 'STOP'].includes(value)) return { key: 'info', label: '已停' };
    return { key: 'info', label: '未知' };
  }

  function metricValue(metrics, key) {
    const rawMetrics = metrics && typeof metrics === 'object' ? metrics : {};
    const source = rawMetrics.control && typeof rawMetrics.control === 'object' ? rawMetrics.control : rawMetrics;
  ...
  function summaryIcon(type) {
    return `<span class="agent-summary-svg ${esc(type)}"></span>`;
  }

  function cardDeviceIcon() {
    return '<span class="agent-card-device-svg"></span>';
  }

  function statusPill(meta) {
    return `<span class="agent-status-pill ${esc(meta.key)}"><span class="agent-status-pill-dot ${esc(meta.key)}"></span>${esc(meta.label)}</span>`;
  }

  function actionLabelWithIcon(label, type) {
    return `<span class="agent-action-inline"><span class="agent-btn-icon ${esc(type)} small"></span><span>${esc(label)}</span></span>`;
  }

  function heartbeatAgeSeconds(agent) {
    const age = Number(agent.last_seen_age_sec);
  ...
  function serviceSummary(agent) {
    ...
    const summary = { total: services.length, online: 0, abnormal: 0, offline: 0 };
    services.forEach((service) => {
      const status = String(service.status || service.run_state || '').toUpperCase();
      if (['ONLINE', 'RUNNING', 'READY', 'SUCCESS'].includes(status)) summary.online += 1;
      else if (['OFFLINE', 'STOPPED', 'TIMEOUT', 'CANCELED'].includes(status)) summary.offline += 1;
  ...
  function matchesFilters(agent) {
    const effectiveStatus = String(agent.effective_status || agent.status || '').toUpperCase();
    if (state.filters.status && effectiveStatus !== state.filters.status) return false;
  ...
  function applyFilters() {
    ...
    if (
      ...
      !state.filters.query &&
      !state.filters.serviceStatus &&
      state.filters.fresh !== 'all' &&
      !state.autoExpandedFreshFilter
    ) {
      state.filters.fresh = 'all';
      state.autoExpandedFreshFilter = true;
      if (elements.filterFresh) elements.filterFresh.value = 'all';
      state.filteredAgents = state.agents.filter(matchesFilters);
      toast('当前活跃筛选隐藏了已注册 Agent，已自动切换为显示全部', 'info');
    }
    const totalPages = Math.max(1, Math.ceil(state.filteredAgents.length / state.pageSize));
  ...
  function renderSummary(summaryResponse, agents) {
    ...
    const logicalOnline = logicalAgents.filter((item) => ['ONLINE', 'READY', 'RUNNING', 'SUCCESS'].includes(String(item.effective_status || item.status || '').toUpperCase())).length;
    const cards = [
      { tone: 'blue', icon: 'device', title: '在线 Agent', value: logicalOnline, desc: '实时在线设备数' },
    ...
    elements.summaryCards.innerHTML = cards.map((card) => `
      <article class="agent-summary-card-mock tone-${esc(card.tone)}">
        <div class="agent-summary-icon">${summaryIcon(card.icon)}</div>
  ...
  function renderResultsMeta() {
    ...
    elements.resultsMeta.innerHTML = `
      ...
      <div class="agent-list-head-right">视图：${state.view === 'card' ? '卡片' : '列表'} · 第 ${state.page} / ${Math.max(1, Math.ceil(total / state.pageSize))} 页</div>
    `;
    elements.selectedCount.textContent = `已选择 ${state.selectedIds.size} 项`;
    elements.paginationMeta.textContent = total ? `共 ${total} 条` : '暂无数据';
  ...
  function renderCard(agent) {
    ...
    const summary = serviceSummary(agent);
    const serviceText = serviceLabel(summary);
    return `
      <article class="agent-mock-card ${selected ? 'selected' : ''} ${status.key === 'warn' ? 'warn' : ''} ${status.key === 'err' ? 'offline' : ''}">
          ...
          <div class="agent-card-main mock">
            <div class="agent-card-title-row mock">
              <div class="agent-card-title-wrap">
                ${cardDeviceIcon()}
                <div class="agent-card-title">${esc(agent.display_name || agent.device_id || agentId)}</div>
                ${statusPill(status)}
              </div>
        ...
        <div class="agent-card-metrics mock">
          <span class="agent-pill cpu">CPU ${esc(metricValue(agent.metrics, 'cpu_percent'))}</span>
          <span class="agent-pill mem">MEM ${esc(metricValue(agent.metrics, 'mem_percent'))}</span>
          <span class="agent-pill disk">DISK ${esc(metricValue(agent.metrics, 'disk_percent'))}</span>
        </div>
        <div class="agent-card-service mock">
          <span>服务 ${esc(summary.online)}/${esc(summary.total)}</span>
          <span class="${serviceText.cls}">${esc(serviceText.text)}</span>
        </div>
        <div class="agent-card-actions mock">
          <a class="agent-card-action-btn" href="/admin/ops-platform/agent-detail?project_id=${encodeURIComponent(state.projectId)}&agent_id=${encodeURIComponent(agentId)}">${actionLabelWithIcon('查看详情', 'detail')}</a>
          <button class="agent-card-action-btn" type="button" data-edit-agent="${esc(agentId)}">${actionLabelWithIcon('编辑', 'edit')}</button>
          <button class="agent-card-action-btn" type="button" data-probe-agent="${esc(agentId)}">${actionLabelWithIcon('探测', 'probe')}</button>
        </div>
      </article>
    `;
  }
        ...
        <table class="agent-table">
          ...
          <tbody>
            ${rows.map((agent) => {
              ...
              const status = statusMeta(agent.effective_status || agent.status);
              const summary = serviceSummary(agent);
              const label = serviceLabel(summary);
              return `
                <tr>
                  <td><input type="checkbox" data-select-agent="${esc(id)}" ${state.selectedIds.has(id) ? 'checked' : ''}></td>
                  <td><div class="table-title">${esc(agent.display_name || id)}</div><div class="table-sub">${esc(agent.device_id || '-')}</div></td>
                  ...
                  <td>CPU ${esc(metricValue(agent.metrics, 'cpu_percent'))} / MEM ${esc(metricValue(agent.metrics, 'mem_percent'))} / DISK ${esc(metricValue(agent.metrics, 'disk_percent'))}</td>
                  <td><span>${esc(summary.online)}/${esc(summary.total)}</span> <span class="${label.cls}">${esc(label.text)}</span></td>
                  <td class="table-actions">
                    <a class="btn ghost" href="/admin/ops-platform/agent-detail?project_id=${encodeURIComponent(state.projectId)}&agent_id=${encodeURIComponent(id)}">详情</a>
                    <button class="btn ghost" type="button" data-edit-agent="${esc(id)}">编辑</button>
                    <button class="btn" type="button" data-probe-agent="${esc(id)}">探测</button>
                  </td>
                </tr>
              `;
  ...
  function renderCards() {
    const rows = pageRows();
    elements.cards.innerHTML = rows.length ? rows.map(renderCard).join('') : '<div class="ops-empty">当前筛选条件下暂无 Agent</div>';
  }
  ...
  async function batchProbe() {
    ...
    if (!agents.length) return toast('请先选择 Agent');
  ...
  async function batchRestart() {
    ...
    toast(`已提交 ${count} 个服务重启请求`, 'success');
    await loadPage();
  }
  ...
  function openEdit(agentId) {
    ...
    elements.agentEditModal.classList.remove('hidden');
  }
  ...
  function closeEdit() {
    elements.agentEditModal.classList.add('hidden');
  }
  ...
  async function saveEdit() {
    ...
    if (isCreating) {
      ...
      if (elements.filterServiceStatus) elements.filterServiceStatus.value = '';
      if (elements.filterFresh) elements.filterFresh.value = 'all';
    }
    closeEdit();
    toast(isCreating ? '新建设备 Agent 成功' : 'Agent 信息保存成功', 'success');
    await loadPage();
  }
  ...
  function toggleMenu(menu, forceOpen) {
    if (!menu) return;
  ...
  function bindStaticActions() {
    ...
    document.getElementById('btnPrevPage').onclick = () => { state.page = Math.max(1, state.page - 1); renderView(); };
    document.getElementById('btnNextPage').onclick = () => {
      state.page = Math.min(Math.max(1, Math.ceil(state.filteredAgents.length / state.pageSize)), state.page + 1);
      renderView();
    };
    ...
    document.getElementById('btnBatchBind').onclick = () => toast('当前后端未提供批量服务绑定接口，入口已保留', 'info');
