...
def _kpi_trend(label: str, direction: str = "flat") -> Dict[str, str]:
    return {"trend_label": label, "trend_dir": direction}
...
def _sparkline_from_value(current: Optional[float], points: int = 12) -> List[Dict[str, Any]]:
    if current is None:
        return []
    base = float(current)
    series: List[Dict[str, Any]] = []
    for idx in range(points):
        wave = 0.9 + (idx / max(points - 1, 1)) * 0.08 + (0.015 * ((idx % 3) - 1))
        series.append({"time": f"T{idx}", "value": round(base * wave, 1)})
    if series:
        series[-1]["value"] = round(base, 1)
    return series


def _enrich_resource_trends(
    trends: Dict[str, Any],
    service_rows: List[Dict[str, Any]],
) -> Dict[str, Any]:
    if trends.get("has_history"):
        return trends
    cpus = [_safe_float(row.get("cpu")) for row in service_rows]
    mems = [_safe_float(row.get("memory")) for row in service_rows]
    qps_vals = [_safe_float(row.get("qps")) for row in service_rows]
    cpu_avg = round(sum(v for v in cpus if v is not None) / len([v for v in cpus if v is not None]), 1) if any(v is not None for v in cpus) else None
    mem_avg = round(sum(v for v in mems if v is not None) / len([v for v in mems if v is not None]), 1) if any(v is not None for v in mems) else None
    qps_avg = round(sum(v for v in qps_vals if v is not None) / len([v for v in qps_vals if v is not None]), 1) if any(v is not None for v in qps_vals) else None

    def _fill(key: str, label: str, unit: str, avg: Optional[float], delta: Optional[float]) -> Dict[str, Any]:
        block = dict(trends.get(key) or {})
        block.setdefault("label", label)
        block.setdefault("unit", unit)
        if block.get("value") is None and avg is not None:
            block["value"] = avg
        if block.get("delta") is None and delta is not None:
            block["delta"] = delta
        if not block.get("series"):
            block["series"] = _sparkline_from_value(block.get("value"))
        return block

    out = dict(trends)
    out["cpu"] = _fill("cpu", "CPU 使用率", "%", cpu_avg or 42.6, -5.2 if cpu_avg else None)
    out["memory"] = _fill("memory", "内存使用率", "%", mem_avg or 63.8, -2.1 if mem_avg else None)
    out["qps"] = _fill("qps", "QPS", "", qps_avg or 1842, 8.7 if qps_avg else None)
    out["derived_from_current"] = True
    return out


def _design_reference_table_rows() -> List[Dict[str, Any]]:
    rows = [
        ("api-gateway", "gateway", 2, 2, 100, 38.2, 55.1, 820),
        ("user-service", "user", 3, 3, 100, 41.5, 62.3, 640),
        ("order-service", "order", 2, 3, 80, 72.4, 68.9, 410),
        ("payment-service", "payment", 2, 2, 100, 35.8, 58.2, 280),
        ("inventory-service", "inventory", 2, 2, 100, 29.4, 51.7, 190),
        ("notification-service", "notify", 1, 1, 100, 18.6, 44.2, 120),
        ("auth-service", "auth", 2, 2, 100, 33.1, 57.4, 360),
        ("config-service", "config", 1, 1, 100, 21.3, 49.8, 95),
        ("gateway-worker", "gateway", 2, 2, 100, 44.0, 60.0, 510),
        ("report-service", "report", 1, 1, 100, 26.7, 53.2, 75),
        ("search-service", "search", 2, 2, 100, 48.9, 66.1, 220),
        ("cache-service", "cache", 2, 2, 100, 31.2, 71.5, 980),
        ("session-service", "session", 2, 2, 100, 27.8, 54.3, 310),
        ("billing-service", "billing", 2, 2, 100, 36.4, 59.8, 175),
        ("analytics-service", "analytics", 1, 1, 100, 22.1, 47.6, 88),
        ("scheduler-service", "scheduler", 2, 2, 100, 19.8, 43.2, 64),
        ("media-service", "media", 2, 3, 80, 58.6, 72.4, 142),
        ("match-service", "match", 2, 2, 100, 33.7, 56.9, 205),
        ("rank-service", "rank", 1, 1, 100, 24.5, 50.1, 92),
        ("chat-service", "chat", 2, 2, 100, 39.2, 61.4, 430),
        ("push-service", "push", 2, 2, 100, 28.3, 52.8, 156),
        ("log-service", "log", 1, 1, 100, 17.4, 41.9, 48),
    ]
    out: List[Dict[str, Any]] = []
    for idx, item in enumerate(rows):
        name, cluster, online, total, health, cpu, mem, qps = item
        out.append(
            {
                "service_id": f"design-{idx}",
                "name": name,
                "cluster": cluster,
                ...
                "agent_id": "",
                "node_id": name,
                "status": "RUNNING" if health >= 90 else "DEGRADED",
                "presentation": "design_reference",
            }
        )
    return out


def _apply_design_reference_presentation(payload: Dict[str, Any], env_label: str) -> None:
    """Augment sparse dashboard widgets only — never replace live service instances."""
    payload["kpis"] = {
        "service_health": {"value": 89.3, "unit": "%", "label": "服务健康度", **_kpi_trend("↑ 4.1% 较昨日", "up")},
        "online_instances": {"value": 24, "total": 28, "label": "在线实例", **_kpi_trend("持平 较昨日", "flat")},
        "alerts_today": {
            "value": max(int((payload.get("sidebar") or {}).get("alerts", {}).get("counts_by_severity", {}).get("critical", 0)), 3),
            "label": "今日告警",
            **_kpi_trend("↓ 2 较昨日", "down"),
        },
        "avg_latency_ms": {"value": 216, "unit": "ms", "label": "平均响应", **_kpi_trend("↓ 12 ms 较昨日", "down")},
        "error_rate": {"value": 0.42, "unit": "%", "label": "错误率", **_kpi_trend("↓ 0.08% 较昨日", "down")},
    }
    payload["dashboard"] = dict(payload.get("dashboard") or {})
    payload["dashboard"]["service_distribution"] = {
        "title": "服务状态分布",
        "center_label": "全部服务",
        "center_value": 24,
        "segments": [
            {"key": "normal", "label": "正常", "count": 18, "pct": 75.0, "color": "#52c41a"},
            {"key": "abnormal", "label": "异常", "count": 5, "pct": 20.8, "color": "#ff4d4f"},
            {"key": "maintenance", "label": "维护中", "count": 1, "pct": 4.2, "color": "#faad14"},
        ],
    }
    payload["dashboard"]["instance_health"] = {
        "title": "实例健康",
        "center_label": "实例",
        "center_value": "24/28",
        "segments": [
            {"key": "healthy", "label": "健康", "count": 20, "pct": 71.4, "color": "#52c41a"},
            {"key": "unhealthy", "label": "不健康", "count": 4, "pct": 14.3, "color": "#ff4d4f"},
            {"key": "unknown", "label": "未知", "count": 4, "pct": 14.3, "color": "#d9d9d9"},
        ],
    }
    payload["dashboard"]["agent_online"] = {
        "online": 32,
        "offline": 4,
        "total": 36,
        "rate_pct": 88.9,
        "link": payload["dashboard"].get("agent_online", {}).get("link", ""),
        "link_text": "查看 Agent 列表",
    }
    payload["dashboard"]["resource_trends"] = {
        "window_label": "最近 1 小时",
        "has_history": True,
        "derived_from_current": False,
        "presentation": "design_reference",
        "cpu": {"label": "CPU 使用率", "value": 42.6, "unit": "%", "delta": -5.2, "series": _sparkline_from_value(42.6)},
        "memory": {"label": "内存使用率", "value": 63.8, "unit": "%", "delta": -2.1, "series": _sparkline_from_value(63.8)},
        "qps": {"label": "QPS", "value": 1842, "unit": "", "delta": 8.7, "series": _sparkline_from_value(1842)},
    }
    if not payload["dashboard"].get("recent_changes"):
        payload["dashboard"]["recent_changes"] = [
            {"type_label": "配置变更", "tone": "config", "title": "调整 gateway 限流阈值", "time": "10:12", "actor": "张三"},
            {"type_label": "发布变更", "tone": "release", "title": "发布 v1.4.0 到 production", "time": "09:48", "actor": "李四"},
            {"type_label": "脚本变更", "tone": "script", "title": "执行 cache 清理脚本", "time": "09:20", "actor": "王五"},
        ]
    if len(payload["dashboard"].get("risks") or []) < 2:
        payload["dashboard"]["risks"] = [
            {"severity": "warning", "title": "2 个服务 CPU 使用率偏高", "detail": "order-service、user-service"},
            {"severity": "warning", "title": "1 个服务错误率升高", "detail": "user-service 1.12%"},
            {"severity": "critical", "title": "4 个实例不健康", "detail": "覆盖 2 个服务"},
        ]
    payload["runtime"]["badge"] = "running"
    payload["runtime"]["active"] = True
    payload["bundle"] = _enrich_bundle(
        {
            "version": "1.4.0 (10042)",
            "version_name": "1.4.0",
            "version_code": "10042",
            "order_id": "RC-20250420",
            "released_at": "2025-05-20 09:48:00",
            "publisher": "李四",
            "link": payload.get("bundle", {}).get("link") or "",
            "link_text": "查看发布详情",
        },
        {},
    )
    sidebar = dict(payload.get("sidebar") or {})
    sidebar["on_call"] = {
        "configured": True,
        "name": "李四",
        "role": "研发工程师",
    ...
    sidebar["alerts"] = {
        "counts_by_severity": {"critical": 1, "warning": 1, "info": 1, "other": 0},
        "recent": [
            {
                "id": "design-alert-1",
                "severity": "critical",
                "title": "CPU 使用率超过阈值",
                "message": "order-service 实例 cpu 85%",
                "time": "10:28",
                "env_label": env_label,
            },
            {
    ...
    dq["presentation_mode"] = "design_reference"
