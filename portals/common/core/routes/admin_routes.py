...
def _admin_layout(content, title, back_href='/admin'):
    ...
    is_project_version_design = 'class="project-version-design-app"' in (content or "")
    content_wrap_class = (
        "w-full px-0 py-0"
        if is_ops_app or is_project_version_design
        else "max-w-6xl mx-auto px-4 py-6 md:py-8"
    )
    header_html = ""
    if not is_project_version_design:
        header_html = f'''
...
def _project_versions_redesign_html(project_id, proj, can_edit, task_stats, recent_tasks, apk_count, project_apk_files=None, is_admin_logged_in=False):
    ...
    selected_stage = (request.args.get('stage') or 'production').strip().lower()
    if selected_stage not in ('dev', 'test', 'production'):
    ...
    all_versions = [v for v in project_versions if isinstance(v, dict)]
    versions = [
        v for v in all_versions
        if str(v.get('stage') or 'dev').strip() == selected_stage
    ]
    unique_groups = len({str(v.get('version_name') or '').strip() for v in versions if str(v.get('version_name') or '').strip()})
    active_versions = len([v for v in versions if _normalize_version_status(v.get('version_status') or 'active') == 'active'])
    build_count = len([v for v in versions if str(v.get('jenkins_job_id') or '').strip()])
    download_count = get_project_download_count(project_id)
    recent_build = next((v for v in sorted(versions, key=lambda item: str(item.get('updated_at') or item.get('created_at') or item.get('version_code') or ''), reverse=True) if str(v.get('version_name') or '').strip()), None)
    latest_build_label = '-'
    if isinstance(recent_build, dict):
        latest_build_label = '%s_%s' % (
            str(recent_build.get('version_name') or '').strip() or 'v?',
            str(recent_build.get('version_code') or '').strip() or 'vc?'
        )
    try:
        from services.test_device_service import list_devices
        device_count = len(list_devices(project_id))
    except Exception:
        device_count = 0
    ...
    def _stage_count(pid, stage_key):
        ...
        return len([row for row in rows if isinstance(row, dict) and str(row.get('stage') or 'dev').strip() == stage_key])

    visible_projects = []
    current_user = _current_username()
    for pid, item in (projects_db or {}).items():
        if not can_view_project(pid, current_user):
            continue
        pname = _clean_display_text((item or {}).get('name'), pid)
        visible_projects.append({
            'id': pid,
            'name': pname,
            'production': _stage_count(pid, 'production'),
            'test': _stage_count(pid, 'test'),
            'dev': _stage_count(pid, 'dev'),
        })
    ...
    for row in visible_projects:
        ...
        production_current = active_project and selected_stage == 'production'
        test_current = active_project and selected_stage == 'test'
        dev_current = active_project and selected_stage == 'dev'
        project_tree_html += (
            '<details class="pv-tree-group %s" %s>'
            '<summary><span class="pv-tree-title">%s</span><i class="fas fa-chevron-down text-[10px] text-slate-400"></i></summary>'
            '<div class="pv-tree-envs">'
            '<a href="/admin/projects/%s/versions?stage=production" class="pv-tree-env %s"><span class="pv-dot pv-green"></span><span>生产环境</span><span class="pv-badge pv-green-badge">%s</span></a>'
            '<a href="/admin/projects/%s/versions?stage=test" class="pv-tree-env %s"><span class="pv-dot pv-yellow"></span><span>测试环境</span><span class="pv-badge pv-yellow-badge">%s</span></a>'
            '<a href="/admin/projects/%s/versions?stage=dev" class="pv-tree-env %s"><span class="pv-dot pv-blue"></span><span>开发环境</span><span class="pv-badge pv-blue-badge">%s</span></a>'
            '</div></details>'
        ) % (
            'is-active' if active_project else '',
            'open' if active_project else '',
            html.escape(row['name']),
            quote(row['id']),
            'is-current' if production_current else '',
            row['production'],
            quote(row['id']),
            'is-current' if test_current else '',
            row['test'],
            quote(row['id']),
            'is-current' if dev_current else '',
            row['dev'],
        )

    metrics = [
        ('渠道数量', len(project_channels), 'fa-store', 'text-blue-600'),
        ('版本组数', unique_groups, 'fa-object-group', 'text-emerald-600'),
        ('有效版本', active_versions, 'fa-cubes', 'text-sky-600'),
        ('构建次数', build_count, 'fa-box-open', 'text-orange-500'),
        ('下载次数(累计)', download_count, 'fa-fire', 'text-rose-500'),
        ('活跃设备', device_count, 'fa-qrcode', 'text-blue-500'),
        ('最近构建', latest_build_label, 'fa-briefcase', 'text-emerald-500'),
    ]
    metrics_html = ''.join(
        '<div class="pv-kpi-card"><div class="pv-kpi-icon %s"><i class="fas %s"></i></div><div><div class="pv-kpi-label">%s</div><div class="pv-kpi-value">%s</div></div></div>' % (
            color, icon, label, html.escape(str(value))
        )
    ...
    recent_versions = sorted(
        versions,
        key=lambda row: str(row.get('updated_at') or row.get('created_at') or row.get('version_code') or ''),
        reverse=True,
    )[:5]
    recent_activity_html = ''.join(
        '<div class="pv-activity-row"><span>%s</span><span>%s</span></div>' % (
            html.escape(str(row.get('version_name') or row.get('version_code') or '-')),
            html.escape(str(row.get('updated_at') or row.get('created_at') or '-')[:16]),
        )
        for row in recent_versions
    ) or '<div class="pv-empty-mini">暂无动态</div>'
    ...
    def _build_version_rows(rows):
        ...
        for row in sorted(
            ...
            out.append({
                ...
                'resource_path': str(row.get('resource_path') or '').strip(),
                'config_path': str(row.get('config_path') or '').strip(),
                'apk_url': str(row.get('apk_url') or '').strip(),
                'resource_url': str(row.get('resource_url') or '').strip(),
                'config_url': str(row.get('config_url') or '').strip(),
                'publish_status': str(row.get('publish_status') or 'draft').strip() or 'draft',
                'active_bundle_id': str(row.get('active_bundle_id') or '').strip(),
                'scope_id': str(row.get('scope_id') or '').strip(),
                'jenkins_job_id': str(row.get('jenkins_job_id') or '').strip(),
            })
        return out

    version_rows = _build_version_rows(versions)
    all_version_rows = _build_version_rows(all_versions)
    version_group_map = {}
    for row in versions:
    ...
    available_channels_js = json.dumps(available_channels, ensure_ascii=False)
    version_rows_js = json.dumps(version_rows, ensure_ascii=False)
    all_version_rows_js = json.dumps(all_version_rows, ensure_ascii=False)
    version_groups_js = json.dumps([
        {
            'version_name': item['version_name'],
            'version_mode': item['version_mode'],
            'recommended': item['recommended'],
            'active_count': item['active_count'],
            'version_code_count': item['version_code_count'],
...
<div class="project-version-design-app" data-project-id="%s" data-default-stage="%s">
  <style>
    ...
    .project-version-design-app .pv-badge { margin-left: auto; min-width: 22px; height: 20px; padding: 0 7px; border-radius: 999px; display: inline-flex; align-items: center; justify-content: center; font-size: 11px; font-weight: 700; }
    .project-version-design-app .pv-green-badge { background: #e8fff2; color: #17a05e; }.project-version-design-app .pv-yellow-badge { background: #fff4df; color: #d48a16; }.project-version-design-app .pv-blue-badge { background: #e8f2ff; color: #347dff; }
    ...
    .project-version-design-app .pv-main-tabs { display: flex; align-items: center; gap: 22px; padding: 0 18px; height: 50px; border-bottom: 1px solid #eef3ff; background: rgba(255,255,255,.96); }
    ...
    .project-version-design-app .pv-code-table tbody td { padding: 9px 16px; border-top: 1px solid #eef3ff; font-size: 13px; color: #334155; vertical-align: middle; }
    ...
    .project-version-design-app .pv-side-card { padding: 18px 16px; }
    ...
    .project-version-design-app .pv-build-list { display: flex; flex-direction: column; gap: 10px; }
    .project-version-design-app .pv-build-row { border: 1px solid #e6edff; border-radius: 14px; background: #fff; padding: 12px 14px; display: flex; align-items: center; justify-content: space-between; gap: 12px; }
    ...
    .project-version-design-app .pv-input, .project-version-design-app .pv-select, .project-version-design-app .pv-textarea { width: 100%%; border: 1px solid #dce6ff; border-radius: 10px; background: #fff; min-height: 34px; padding: 0 11px; font-size: 12px; color: #1e293b; }
    .project-version-design-app .pv-textarea { min-height: 66px; padding-top: 8px; padding-bottom: 8px; resize: vertical; }
    .project-version-design-app .pv-dropdown-shell { position: relative; display: inline-flex; flex-direction: column; align-items: flex-start; }
    .project-version-design-app .pv-channel-dropdown { position: absolute; left: 0; top: 100%%; margin-top: 8px; min-width: 180px; background: #fff; border: 1px solid #dbe5ff; border-radius: 14px; box-shadow: 0 20px 40px rgba(15, 23, 42, .12); padding: 8px 0; z-index: 50; }
    .project-version-design-app .pv-channel-dropdown button { width: 100%%; text-align: left; padding: 8px 14px; font-size: 13px; color: #334155; }
    .project-version-design-app .pv-channel-dropdown button:hover { background: #f8fbff; }
    .project-version-design-app .pv-channel-dropdown p { padding: 8px 14px; font-size: 12px; color: #94a3b8; }
    .project-version-design-app .pv-inline-two { display: grid; grid-template-columns: repeat(2, minmax(0,1fr)); gap: 10px; }
    .project-version-design-app .pv-inline-three { display: grid; grid-template-columns: repeat(3, minmax(0,1fr)); gap: 10px; }
    .project-version-design-app .pv-dock-actions { margin-top: auto; display: flex; gap: 10px; }
    .project-version-design-app .pv-secondary { height: 34px; border-radius: 10px; border: 1px solid #dce6ff; background: #fff; color: #51637f; font-size: 12px; padding: 0 14px; }
    .project-version-design-app .pv-primary { height: 34px; border-radius: 10px; border: 0; background: linear-gradient(180deg, #3f7bff, #2b68ff); color: #fff; font-size: 12px; font-weight: 700; padding: 0 16px; box-shadow: 0 10px 18px rgba(47, 108, 255, .18); }
    ...
    .project-version-design-app .pv-gm-candidate-radio { width: 16px; height: 16px; border-radius: 999px; border: 2px solid #c7d8ff; position: relative; }
    ...
    .project-version-design-app .pv-gm-status-pill.success { background: rgba(40,199,111,.14); color: #0f9f53; }
    .project-version-design-app .pv-gm-status-pill.warn { background: rgba(255,183,77,.18); color: #d07b00; }
    .project-version-design-app .pv-gm-status-pill.error { background: rgba(255,101,132,.14); color: #d94163; }
    .project-version-design-app .pv-gm-alert-list { margin-top: 12px; display: grid; gap: 8px; }
    .project-version-design-app .pv-gm-alert-item { border-radius: 14px; padding: 10px 12px; font-size: 12px; line-height: 1.55; }
    ...
    @media (max-width: 1120px) {
      ...
      .project-version-design-app .pv-app-nav { display: none; }
      .project-version-design-app .pv-body { grid-template-columns: 1fr; }
      .project-version-design-app .pv-side { grid-template-columns: 1fr; }
      .project-version-design-app .pv-kpis { grid-template-columns: repeat(2, minmax(0, 1fr)); }
    }
  </style>
  <div class="pv-shell">
    <aside class="pv-app-nav">
      <div class="pv-brand">
        <div class="pv-brand-mark">G</div>
        <div class="pv-brand-copy"><b>运维平台</b><span>GameOps Console</span></div>
      </div>
      <div class="pv-nav-group">
        <a href="/admin" class="pv-nav-item"><i class="fas fa-house"></i><span>工作台</span></a>
        <a href="/admin/dashboard" class="pv-nav-item"><i class="fas fa-chart-pie"></i><span>总览看板</span></a>
      </div>
      <div class="pv-nav-group">
        <div class="pv-nav-title">项目管理</div>
        <a href="/admin/projects" class="pv-nav-item"><i class="fas fa-table-list"></i><span>项目列表</span></a>
      ...
      <div class="pv-tree-footer">
        <a href="/logout" class="pv-nav-item"><i class="fas fa-angles-left"></i><span>收起菜单</span></a>
      </div>
    </aside>
    <div class="pv-main">
      <div class="pv-topbar">
      ...
      <div class="pv-body">
        <aside class="pv-panel pv-project-tree">
          ...
          <div class="pv-tree-footer"><button type="button" class="pv-blue-btn w-full justify-center" onclick="pvOpenModal('topology')"><i class="fas fa-plus"></i><span>新建拓扑图</span></button></div>
        </aside>
          ...
          <div class="pv-panel pv-workbench">
            ...
            <div class="pv-toolbar">
              <div class="pv-toolbar-left">
                <span class="text-[12px] font-semibold text-[#4f6b97]">当前渠道：</span>
                <button type="button" class="pv-segment is-active" data-pv-scope="all">全部</button>
                <button type="button" class="pv-segment" data-pv-platform="android">Android</button>
                <button type="button" class="pv-segment" data-pv-platform="ios">iOS</button>
              </div>
            ...
            <div class="pv-workbench-body">
              ...
              <div id="pvPanelDevices" class="pv-content-panel">
                <div class="pv-panel pv-side-card">
                  <h3>测试设备</h3>
                  <p class="text-sm text-slate-500">测试设备用于 `version-resolve` 精确选版，按设备、平台、阶段控制客户端拿到的版本。</p>
                  <div class="mt-4 space-y-3">
                    <div class="pv-inline-two">
                      <div class="pv-field"><label>设备 ID</label><input id="pvDeviceId" class="pv-input" placeholder="Android ID / IDFV"></div>
                      <div class="pv-field"><label>平台</label><select id="pvDevicePlatform" class="pv-select"><option value="android">Android</option><option value="ios">iOS</option></select></div>
                    </div>
                    <div class="pv-inline-two">
                      <div class="pv-field"><label>阶段</label><select id="pvDeviceStage" class="pv-select"><option value="production">prod</option><option value="test">test</option><option value="dev">dev</option></select></div>
                      <div class="pv-field"><label>备注名</label><input id="pvDeviceLabel" class="pv-input" placeholder="如：小米真机"></div>
                    </div>
                    <div class="pv-field"><label>说明</label><textarea id="pvDeviceNotes" class="pv-textarea" placeholder="用途、负责人、灰度说明"></textarea></div>
                    <input type="hidden" id="pvDeviceEditId" value="">
                    <div class="pv-dock-actions">
                      <button type="button" class="pv-secondary" id="pvDeviceResetBtn">重置表单</button>
                      <button type="button" class="pv-primary" id="pvDeviceSaveBtn">保存设备</button>
                    </div>
                    <div id="pvDeviceList" class="pv-build-list"></div>
                    <a href="/api/admin/projects/%s/test-devices" target="_blank" class="pv-white-btn"><i class="fas fa-list"></i><span>查看 JSON</span></a>
                  </div>
                </div>
              </div>
              <div id="pvPanelDownloads" class="pv-content-panel">
                <div class="pv-panel pv-side-card">
                  <h3>下载中心</h3>
                  %s
                </div>
        ...
        <aside class="pv-side">
          <div class="pv-panel pv-side-card">
            ...
            <div class="pv-side-kv">
              ...
              <span>项目阶段：</span><b>%s</b>
              <span>创建时间：</span><b>%s</b>
              <span>负责人：</span><b>%s</b>
            </div>
          </div>
          <div class="pv-panel pv-side-card">
  ...
  <div class="pv-modal-overlay" data-pv-modal="topology" onclick="if(event.target===this){ pvCloseModal('topology'); }">
    <div class="pv-panel pv-modal-card" onclick="event.stopPropagation()">
      <button type="button" class="pv-modal-close" onclick="pvCloseModal('topology')"><i class="fas fa-times"></i></button>
          <h4>新建拓扑图</h4>
          <div class="pv-field"><label>所属项目</label><input class="pv-input" value="%s" readonly></div>
          <div class="pv-field"><label>拓扑名称</label><input id="pvTopologyName" class="pv-input" value="%s_topology_v4"></div>
  ...
  <div class="pv-modal-overlay" data-pv-modal="compare" onclick="if(event.target===this){ pvCloseModal('compare'); }">
    <div class="pv-panel pv-modal-card pv-modal-narrow" onclick="event.stopPropagation()">
      <button type="button" class="pv-modal-close" onclick="pvCloseModal('compare')"><i class="fas fa-times"></i></button>
          <h4>版本对比</h4>
          <div class="pv-field"><label>左侧版本组</label><select id="pvCompareLeft" class="pv-select"></select></div>
          <div class="pv-field"><label>右侧版本组</label><select id="pvCompareRight" class="pv-select"></select></div>
          <div id="pvCompareResult" class="rounded-xl border border-[#dce6ff] bg-[#f8fbff] px-3 py-3 text-xs text-slate-600">请选择两个版本组进行对比。</div>
          <div class="pv-dock-actions">
            <button type="button" class="pv-secondary" onclick="pvCloseModal('compare')">关闭</button>
            <button type="button" class="pv-primary" id="pvCompareRunBtn">开始对比</button>
          </div>
    </div>
  </div>
  ...
  <div class="pv-modal-overlay" data-pv-modal="qr-download" onclick="if(event.target===this){ pvCloseModal('qr-download'); }">
    <div class="pv-panel pv-modal-card pv-modal-narrow" onclick="event.stopPropagation()">
      <button type="button" class="pv-modal-close" onclick="pvCloseModal('qr-download')"><i class="fas fa-times"></i></button>
          <h4>扫码下载</h4>
          <div class="pv-qr-shell">
            <div class="pv-qr-box"></div>
            <div class="text-center">
              <div class="font-semibold text-[#173057]">%s</div>
              <div class="text-xs text-slate-500">%s</div>
            </div>
          </div>
          <div class="pv-dock-actions mt-4">
            <button type="button" class="pv-primary w-full" %s>打开二维码</button>
          </div>
    </div>
  </div>
  <script>
    function pvOpenModal(name){
      var el = document.querySelector('.project-version-design-app [data-pv-modal="' + name + '"]');
      if(el){ el.classList.add('is-open'); }
    }
    function pvCloseModal(name){
      var el = document.querySelector('.project-version-design-app [data-pv-modal="' + name + '"]');
      if(el){ el.classList.remove('is-open'); }
    }
    function switchPvPanel(target){
    ...
    function pvApplyFilters(){
      var q = String((document.getElementById('pvVersionSearch') || {}).value || '').trim().toLowerCase();
    ...
    function pvRefreshSummaries(){
      if(!window.__PV_VERSION_ROWS){ return; }
      var versionGroups = [];
      var seenGroups = {};
      window.__PV_VERSION_ROWS.forEach(function(row){
        var versionName = String(row.version_name || '').trim();
        if(versionName && !seenGroups[versionName]){
          seenGroups[versionName] = true;
          versionGroups.push(versionName);
        }
      });
      var targetSel = document.getElementById('pvQuickVersionCodeTarget');
      if(targetSel){
        targetSel.innerHTML = window.__PV_VERSION_ROWS.map(function(row){
          var label = (row.version_name || '-') + ' / ' + (row.version_code || '-') + ' / ' + (row.platform_label || row.platform || '-');
          return '<option value=\"' + String(row.id || '') + '\">' + label + '</option>';
        }).join('') || '<option value=\"\">暂无 VersionCode</option>';
      }
      var compareLeft = document.getElementById('pvCompareLeft');
      var compareRight = document.getElementById('pvCompareRight');
      if(compareLeft && compareRight){
        var compareOptions = versionGroups.map(function(name){
          return '<option value=\"' + name + '\">' + name + '</option>';
        }).join('');
        compareLeft.innerHTML = compareOptions || '<option value=\"\">暂无版本组</option>';
        compareRight.innerHTML = compareOptions || '<option value=\"\">暂无版本组</option>';
    ...
    function pvRenderCompare(){
      ...
      var right = (document.getElementById('pvCompareRight') || {}).value || '';
      var result = document.getElementById('pvCompareResult');
      if(!result){ return; }
      if(!left || !right){
        result.textContent = '请选择两个版本组进行对比。';
        return;
      }
      if(left === right){
        result.textContent = '请选择两个不同的版本组。';
        return;
      }
      var groups = window.__PV_VERSION_GROUPS || [];
      var leftRow = groups.find(function(item){ return String(item.version_name || '') === left; });
      var rightRow = groups.find(function(item){ return String(item.version_name || '') === right; });
      if(!leftRow || !rightRow){
        result.textContent = '未找到可对比的版本组数据。';
        return;
      }
      result.innerHTML =
        '<div class="space-y-2">'
        + '<div><b>' + left + '</b> vs <b>' + right + '</b></div>'
        + '<div>VersionCode 数量：' + String(leftRow.version_code_count || 0) + ' / ' + String(rightRow.version_code_count || 0) + '</div>'
        + '<div>有效版本：' + String(leftRow.active_count || 0) + ' / ' + String(rightRow.active_count || 0) + '</div>'
        + '<div>下载次数：' + String(leftRow.download_total || 0) + ' / ' + String(rightRow.download_total || 0) + '</div>'
        + '<div>版本模式：' + String(leftRow.version_mode || '-') + ' / ' + String(rightRow.version_mode || '-') + '</div>'
        + '<div>最近更新：' + String(leftRow.latest_updated_at || '-') + ' / ' + String(rightRow.latest_updated_at || '-') + '</div>'
        + '</div>';
    }
    function pvResetDeviceForm(){
      ['pvDeviceEditId', 'pvDeviceId', 'pvDeviceLabel', 'pvDeviceNotes'].forEach(function(id){
        var el = document.getElementById(id);
        if(el){ el.value = ''; }
      });
      var platformEl = document.getElementById('pvDevicePlatform');
      var stageEl = document.getElementById('pvDeviceStage');
      if(platformEl){ platformEl.value = 'android'; }
      if(stageEl){ stageEl.value = document.querySelector('.project-version-design-app').getAttribute('data-default-stage') || 'production'; }
    }
    function pvLoadTestDevices(){
      var wrap = document.getElementById('pvDeviceList');
      if(!wrap){ return; }
      wrap.innerHTML = '<div class="text-sm text-slate-400">加载中…</div>';
      fetch('/api/admin/projects/' + encodeURIComponent(%s) + '/test-devices', { credentials:'same-origin' })
        .then(function(r){ return r.json(); })
        .then(function(d){
          window.__PV_TEST_DEVICES = (d && d.devices) ? d.devices : [];
          if(!window.__PV_TEST_DEVICES.length){
            wrap.innerHTML = '<div class="pv-empty-state">当前还没有测试设备，保存后会立刻出现在这里。</div>';
            return;
          }
          var stageLabel = { production:'prod', test:'test', dev:'dev' };
          wrap.innerHTML = window.__PV_TEST_DEVICES.map(function(row, idx){
            return '<div class="pv-build-row">'
              + '<div><div class="pv-build-title">' + String(row.label || row.device_id || '-') + '</div><div class="pv-build-meta">' + String(row.device_id || '-') + ' / ' + String((row.platform || 'android').toUpperCase()) + ' / ' + String(stageLabel[row.stage] || row.stage || '-') + '</div></div>'
              + '<div class="flex items-center gap-2">'
              + '<button type="button" class="pv-icon-btn" data-device-edit-idx="' + String(idx) + '" title="编辑设备"><i class="fas fa-pen"></i></button>'
              + '<button type="button" class="pv-icon-btn danger" data-device-delete-id="' + String(row.id || '') + '" title="删除设备"><i class="fas fa-trash"></i></button>'
              + '</div></div>';
          }).join('');
        })
        .catch(function(){
          wrap.innerHTML = '<div class="pv-empty-state">设备列表加载失败，请刷新后重试。</div>';
        });
    }
    function pvSaveTestDevice(){
      var payload = {
        id: (document.getElementById('pvDeviceEditId') || {}).value || '',
        device_id: ((document.getElementById('pvDeviceId') || {}).value || '').trim(),
        platform: (document.getElementById('pvDevicePlatform') || {}).value || 'android',
        stage: (document.getElementById('pvDeviceStage') || {}).value || 'production',
        label: ((document.getElementById('pvDeviceLabel') || {}).value || '').trim(),
        notes: ((document.getElementById('pvDeviceNotes') || {}).value || '').trim()
      };
      if(!payload.device_id){
        alert('设备 ID 不能为空');
        return;
      }
      fetch('/api/admin/projects/' + encodeURIComponent(%s) + '/test-devices', {
        method:'POST',
    ...
    function pvEditTestDeviceByIdx(idx){
      var row = (window.__PV_TEST_DEVICES || [])[idx];
      if(!row){ return; }
      document.getElementById('pvDeviceEditId').value = row.id || '';
      document.getElementById('pvDeviceId').value = row.device_id || '';
      document.getElementById('pvDevicePlatform').value = row.platform || 'android';
      document.getElementById('pvDeviceStage').value = row.stage || 'production';
      document.getElementById('pvDeviceLabel').value = row.label || '';
      document.getElementById('pvDeviceNotes').value = row.notes || '';
      switchPvPanel('devices');
    }
    function pvDeleteTestDevice(id){
      if(!id || !confirm('确定删除该测试设备吗？')){ return; }
      fetch('/api/admin/projects/' + encodeURIComponent(%s) + '/test-devices/' + encodeURIComponent(id), {
        method:'DELETE',
        credentials:'same-origin'
      }).then(function(r){ return r.json(); }).then(function(d){
        if(d.error){ alert(d.error); return; }
    ...
    function pvQuickCreateVersionGroup(){
      ...
      var payload = {
        ...
        stage: document.getElementById('pvQuickVersionStage').value || 'production',
        platform: 'android',
        version_status: document.getElementById('pvQuickVersionStatus').value || 'active',
    ...
    function pvQuickSaveVersionCode(){
      var target = document.getElementById('pvQuickVersionCodeTarget').value || '';
      if(!target){ alert('请先选择 VersionCode 记录'); return; }
      var row = (window.__PV_VERSION_ROWS || []).find(function(item){ return String(item.id || '') === target; });
    ...
    function pvOpenQrPreview(){
      location.href = '/download-center?project=' + encodeURIComponent(%s);
    }
    function toggleAddChannelDropdown(kind){
      ...
      if(dd.classList.contains('hidden')){
        ...
        var list = %s || [];
        if(!list.length){
          dd.innerHTML = '<p>无更多渠道可添加</p>';
          return;
        }
        list.forEach(function(item){
          dd.innerHTML += '<button type=\"button\" class=\"add-channel-btn\" data-channel-id=\"' + item.id + '\">' + item.name + '</button>';
        });
      } else {
        dd.classList.add('hidden');
      }
    }
    function addProjectChannel(cid){
      fetch('/admin/projects/' + encodeURIComponent(%s) + '/channels/add', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({channel_id:cid}), credentials:'same-origin' }).then(function(r){ return r.json(); }).then(function(d){ if(d.error){ alert(d.error); return; } location.reload(); });
    }
    function removeProjectChannel(cid){
      if(!confirm('确定从项目中移除此渠道？')) return;
      fetch('/admin/projects/' + encodeURIComponent(%s) + '/channels/remove', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({channel_id:cid}), credentials:'same-origin' }).then(function(r){ return r.json(); }).then(function(d){ if(d.error){ alert(d.error); return; } location.reload(); });
    }
    function pvDeleteVersion(id){
    ...
    (function(){
      ...
      document.querySelectorAll('[data-pv-modal-open]').forEach(function(btn){
        btn.addEventListener('click', function(){
          pvOpenModal(btn.getAttribute('data-pv-modal-open'));
        });
      });
      window.__PV_VERSION_ROWS = %s;
      window.__PV_ALL_VERSION_ROWS = %s;
      ...
      var globalSearch = document.getElementById('pvGlobalSearch');
      if(globalSearch){
        globalSearch.addEventListener('input', function(){
          var q = String(globalSearch.value || '');
          var treeInput = document.getElementById('pvProjectTreeSearch');
          var versionInput = document.getElementById('pvVersionSearch');
          if(treeInput){ treeInput.value = q; treeInput.dispatchEvent(new Event('input')); }
          if(versionInput){ versionInput.value = q; pvApplyFilters(); }
        });
      }
      document.querySelectorAll('[data-pv-platform]').forEach(function(btn){
        btn.addEventListener('click', function(){
          document.querySelectorAll('[data-pv-platform]').forEach(function(other){ other.classList.remove('is-active'); });
          btn.classList.add('is-active');
          var hidden = document.getElementById('pvPlatformFilterHidden');
          if(hidden){ hidden.value = btn.getAttribute('data-pv-platform') || 'all'; }
          pvApplyFilters();
        });
      });
      document.querySelector('[data-pv-scope=\"all\"]').addEventListener('click', function(){
        document.querySelectorAll('[data-pv-platform]').forEach(function(other){ other.classList.remove('is-active'); });
        var hidden = document.getElementById('pvPlatformFilterHidden');
      ...
      hiddenInput.id = 'pvPlatformFilterHidden';
      hiddenInput.value = 'all';
      document.body.appendChild(hiddenInput);
      document.addEventListener('click', function(e){
        var addBtn = e.target.closest('.add-channel-btn');
        if(addBtn){ addProjectChannel(addBtn.getAttribute('data-channel-id') || ''); return; }
        var removeBtn = e.target.closest('.remove-channel-btn');
        if(removeBtn){ removeProjectChannel(removeBtn.getAttribute('data-channel-id') || ''); return; }
        var editBtn = e.target.closest('[data-edit-version]');
        if(editBtn){
          var target = editBtn.getAttribute('data-edit-version') || '';
          document.getElementById('pvQuickVersionCodeTarget').value = target;
          var row = (window.__PV_VERSION_ROWS || []).find(function(item){ return String(item.id || '') === target; });
          if(row){
            document.getElementById('pvQuickVersionCodeValue').value = row.version_code || '';
            document.getElementById('pvQuickVersionCodeStatus').value = row.version_status || 'active';
            document.getElementById('pvQuickVersionCodeChangelog').value = row.notes || '';
          }
          pvOpenModal('version-code');
          return;
        }
        ...
        if(gmRowBtn){
          ...
          var selected = (window.__PV_ALL_VERSION_ROWS || []).find(function(item){ return String(item.id || '') === gmId; }) || null;
          pvRenderGmSelection(selected);
          document.querySelectorAll('.pv-gm-candidate-row').forEach(function(row){
            row.classList.toggle('is-selected', row.getAttribute('data-gm-version-id') === gmId);
          });
          return;
        }
        ...
        if(deviceDeleteBtn){ pvDeleteTestDevice(deviceDeleteBtn.getAttribute('data-device-delete-id') || ''); return; }
      });
      var projectSearch = document.getElementById('pvProjectTreeSearch');
      if(projectSearch){
        projectSearch.addEventListener('input', function(){
          var q = String(projectSearch.value || '').trim().toLowerCase();
          document.querySelectorAll('#pvProjectTree .pv-tree-group').forEach(function(group){
