...
# 项目根与 Jenkins 集成目录（便于迁移：jenkins-clone 放入 apk-site 后直接可用）
_APP_DIR = os.path.dirname(os.path.abspath(__file__))
APK_SITE_ROOT = _APP_DIR
JENKINS_CLONE_DIR = os.path.join(_APP_DIR, 'jenkins-clone')
...
def _jenkins_credentials_path():
    return os.path.join(_APP_DIR, 'jenkins_credentials.json')
...
def _resolve_path(val, base=_APP_DIR):
    """相对路径转为绝对路径。"""
    if not val:
        return ''
    val = str(val).strip()
